...
(function () {
  ...
  var DAYS_ID = 'mdc-cronometro-days';
  ...
  function findTargetButton() {
    ...
    for (var i = 0; i < buttons.length; i++) {
      ...
      for (var j = 0; j < images.length; j++) {
        ...
        if (
          src.indexOf('cronometro-mobile.png') !== -1 ||
          src.indexOf('cronometro-desktop.png') !== -1
        ) {
          hasTargetImage = true;
          break;
        }
  ...
  function injectStyles() {
    ...
    style.textContent =
      ...
      ".mdc-cronometro-time{display:flex;align-items:flex-start;gap:18px;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-weight:700;font-size:85px;line-height:92px;color:#fff;white-space:nowrap;}" +
      '.mdc-cronometro-unit{display:flex;flex-direction:column;align-items:center;}' +
      '.mdc-cronometro-value{min-width:100px;text-align:center;}' +
      '.mdc-cronometro-label{font-size:14px;font-weight:400;line-height:1.2;color:#fff;text-align:center;letter-spacing:0.05em;text-transform:uppercase;margin-top:4px;}' +
      '.mdc-cronometro-sep{width:20px;text-align:center;padding-top:0;}' +
      ...
      '.mdc-cronometro-time{font-size:clamp(46px,6vw,66px);line-height:1.05;gap:clamp(6px,0.9vw,12px);}' +
      '.mdc-cronometro-value{min-width:clamp(58px,7.5vw,90px);}' +
      '.mdc-cronometro-label{font-size:clamp(10px,1.1vw,13px);}' +
      '.mdc-cronometro-sep{width:clamp(10px,1.5vw,18px);}' +
      '}' +
      ...
      '.mdc-cronometro-time{font-size:52px;line-height:56px;gap:8px;}' +
      '.mdc-cronometro-value{min-width:60px;}' +
      '.mdc-cronometro-label{font-size:11px;}' +
      '.mdc-cronometro-sep{width:14px;}' +
      '.mdc-cronometro-badge{width:72px;height:74px;flex-basis:72px;}' +
      '}' +
      ...
      '.mdc-cronometro-copy-rest{color:#fff;}' +
      '.mdc-cronometro-time{grid-area:time;justify-content:center;font-size:42px;line-height:46px;gap:6px;}' +
      '.mdc-cronometro-value{min-width:50px;}' +
      '.mdc-cronometro-label{font-size:10px;}' +
      '.mdc-cronometro-sep{width:10px;}' +
      '}';
  ...
  function createOverlay() {
    return (
      '<div class="mdc-cronometro-overlay" id="' +
      OVERLAY_ID +
      '">' +
      '  <div class="mdc-cronometro-content">' +
      '    <div class="mdc-cronometro-left">' +
      '      <div class="mdc-cronometro-badge">' +
      createBadgeIconHTML() +
      '</div>' +
      ...
      '      <div class="mdc-cronometro-unit"><span class="mdc-cronometro-value" id="' +
      DAYS_ID +
      '">00</span><span class="mdc-cronometro-label">Dias</span></div>' +
      '      <span class="mdc-cronometro-sep">:</span>' +
      '      <div class="mdc-cronometro-unit"><span class="mdc-cronometro-value" id="' +
      HOURS_ID +
      '">00</span><span class="mdc-cronometro-label">Horas</span></div>' +
      '      <span class="mdc-cronometro-sep">:</span>' +
      '      <div class="mdc-cronometro-unit"><span class="mdc-cronometro-value" id="' +
      MINUTES_ID +
      '">00</span><span class="mdc-cronometro-label">Minutos</span></div>' +
      '      <span class="mdc-cronometro-sep">:</span>' +
      '      <div class="mdc-cronometro-unit"><span class="mdc-cronometro-value" id="' +
      SECONDS_ID +
      '">00</span><span class="mdc-cronometro-label">Segundos</span></div>' +
      '    </div>' +
      '  </div>' +
      '</div>'
    );
  ...
  function updateCountdown() {
    ...
    if (!daysEl || !hoursEl || !minutesEl || !secondsEl || !messageEl) return;
