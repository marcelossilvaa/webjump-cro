(function () {
  ...
  const checkIfDomReady = () => {
    ...
    if (deviceWidth > 767) {
      console.log(
        "[AT] Device width is greater than 767px, skipping floating header."
      );
      return;
    }
  ...
  const checkIfWrapperExists = () => {
    ...
    if (offersWrapper) {
      console.log(
        "[AT] Offers wrapper exists, proceeding with floating header initialization."
      );
  ...
  const initializeFloatingHeader = () => {
    ...
    function removeDefaultWrapper() {
      const offersWrapper = document.querySelector(SELECTORS.offersWrapper);
      const offersCardsWrapper = offersWrapper
        ? offersWrapper.children[5]
        : null;
    ...
    function injectFloatingHeader() {
      ...
      floatingHeader
        .querySelectorAll(".floating-header__link")
        .forEach((link) => {
    ...
    function injectCustomStyles() {
      ...
      style.textContent = `
                ...
                .inject-floating-header {
                    ...
                    top: 56px;
                    width: 100%;
                    background: linear-gradient(161deg, #008BC4 15%, #5BABE6 40%);
                    padding: 12px 16px;
                    display: flex;
                    height: 81px;
