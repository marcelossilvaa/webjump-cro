    ...
    function initTariffRecommendation() {
        ...
        function observerFlights() {
            const observerFlightsWrapper = new MutationObserver((mutations) => {
                ...
                if(!stillMoneyPaymentFlight) {
                    console.log("[AT] Tariff Recommendation: Flight selection changed to non Money Payment flight.");
        ...
        function addRecommendationTagsForNational(avilableTarrifs) {
            if(!avilableTarrifs || avilableTarrifs.length === 0) {
                console.log("[AT] Tariff Recommendation: function doesn't receive avilableTarrifs as parameter.");
                return;
            }

            const filteredTariffs = filterTariffsByName(avilableTarrifs, TARIFF_RECOMMENDATION_FOR_NATIONAL);
        ...
        function getTierOfUser() {
            const userClassification = document.querySelector(SELECTORS.tierLabelOnHeader)?.textContent.trim().toLowerCase();
        ...
        function getOnlyNotSoldOutTariffs(flightCards) {
            if(!flightCards || flightCards.length === 0) {
                console.log("[AT] Tariff Recommendation: function doesn't receive flight cards as parameter.");
                return [];
            }
