(function () {
  const checkIfDomReady = () => {
    ...
    if (!isCarPage) {
      console.log(
        "[AT] Page is not a car page, skipping filters optimization."
      );
      return;
    }
  ...
  const initilizeFiltersOptimization = () => {
    const originalContentWrapper = document.querySelector(
      ".styles__CarsSearchLayoutWrapper-sc-fdgbpv-0"
    );

    if (!originalContentWrapper) {
      console.log(
        "[AT] Original content wrapper not found: .styles__CarsSearchLayoutWrapper-sc-fdgbpv-0"
      );
      return;
    }
    ...
    // Dispara somente "expand" ao clicar no cabeçalho do filtro
    function bindFilterExpandAnalytics() {
      ...
      wrapper.addEventListener(
        ...
        function (ev) {
          ...
          var filterName = normalizeText(
            nameEl ? nameEl.textContent : "Sem rotulo"
          );

          analyticsEvent("AT_filtros_vertical:click " + filterName); // event90 + eVar82
        },
        true
      );
    }
    ...
    function changeLayoutToLoadingOrError() {
      const resultsLoadingWrapper = document.querySelector(
        ".styles__Container-sc-kv8lfe-0"
      );

      if (!resultsLoadingWrapper) {
        requestAnimationFrame(changeLayoutToLoadingOrError);
        console.log(
          "[AT] Results loading wrapper not found, waiting for it to appear."
        );
        return;
      }
      ...
      const loadingObserver = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
          if (
            ...
            const hasError = resultsLoadingWrapper.querySelector(
              ".styles__ContainerError-sc-kv8lfe-1"
            );

            if (hasError) {
              console.log(
                "[AT] Error detected in results loading wrapper, skipping filters optimization."
              );
            ...
            if (loadingWrapper) {
              console.log(
                "[AT] Loading wrapper detected, after content changes applying loading classes."
              );
              return;
            }
            ...
            reorganizeClasses();
            console.log(
              "[AT] New content rendered, filters optimization applied."
            );
          }
        });
      });
      ...
      const loadingObserver = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
          if (
            ...
            const hasError = originalContentWrapper.querySelector(
              ".styles__ContainerError-sc-kv8lfe-1"
            );
    ...
    function changeHandlersForFilterCarousel() {
      const nextArrow = document.querySelector(
        ".styles__Arrow-sc-3qprdy-2.ekfkGu.arrow-button"
      );
      const prevArrow = document.querySelector(
        ".styles__Arrow-sc-3qprdy-2.duXrFA.arrow-button"
      );
