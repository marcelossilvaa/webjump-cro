(function () {
  ...
  // Função para remover hover do link
  function removeLinkHover(link) {
    if (!link.hasAttribute('data-hover-removed')) {
      link.style.setProperty('outline', 'none', 'important');
      link.style.setProperty('margin', 'initial', 'important');
      link.style.setProperty('border', 'none', 'important');
      link.style.setProperty('border-radius', 'initial', 'important');
      link.setAttribute('data-hover-removed', 'true');
    }
  }
  ...
  function customizeOldHeader(header) {
    ...
    if (headerLink) {
      // Remover hover do link
      removeLinkHover(headerLink);
      ...
      if (logoContainer) {
        ...
        if (oldSvg && !oldSvg.hasAttribute('data-logo-replaced')) {
          ...
          if (isHomePage()) {
            newSvg.style.setProperty('scale', '0.50', 'important');
          }
  ...
  function customizeHeader() {
    ...
    // Remover hover de todos os links com classe coJFpJ (dentro do header)
    const linksWithHover = header.querySelectorAll('a.coJFpJ');
    linksWithHover.forEach((link) => {
      removeLinkHover(link);
    });

    // Remover hover de todos os links do logo (azul-logo ou links que contêm SVG customizado)
    const logoLinks = header.querySelectorAll(
      'a.azul-logo, a[href*="home"], a[href*="azul-friday"]'
    );
