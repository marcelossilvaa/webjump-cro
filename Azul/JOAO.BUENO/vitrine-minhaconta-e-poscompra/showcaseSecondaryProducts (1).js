(function () {
  ...
  const CTA_PRODUCTS_SELECTOR_DESCRIPTION =
    ".sc-fVLGaz .cLEazU.richTextContainer";
  ...
  const PRODUCTS = [
    ...
    },
    {
      ...
      identifier: "Hotel",
      description:
        "Sua viagem com conforto e desconto!<br/> Use o cupom: CLIENTEAZUL",
  ...
  function initShowcaseSecondaryProducts() {
    ...
    const currentProductsElement = document.querySelectorAll(
    ...
    if (deviceWidth < 601) {
      console.log(
        "[AT] Device width is less than 601px, skipping filters optimization."
      );

      if (!targetContainer || !currentProductsElement) {
        console.log(
          "[AT] Waiting for target container and current products slider to be available..."
        );
        requestAnimationFrame(initShowcaseSecondaryProducts);
      ...
      console.log(
        "[AT] Device width is greater than 601px, skipping filters optimization."
      );

      if (
        !targetContainer ||
        !slickTrackOfCurrentProducts ||
        !currentElementDotList
      ) {
        console.log(
          "[AT] Waiting for target container and current products slider to be available..."
        );
    ...
    function forceDomLoadingOfCurrentProductsSlider() {
      const currentElementArrowsList =
        document.querySelectorAll(".sc-dXxSUK.ktjWwA");
      ...
      const intervalCurrentArrows = setInterval(() => {
        if (i >= currentElementDotList.childElementCount) {
          currentElementDotList.childNodes[0]
            ?.querySelector("svg")
    ...
    function addShowcaseSecondaryProducts() {
      ...
      injectShowcaseWrapper.classList.add(
        "injectShowcaseSecondaryProductsWrapper"
      );
      ...
      FIXED_PRODUCTS.forEach((product) => {
        product.buttonTarget = document.querySelector(
          product.selectorTriggerClick
        );

        if (product.buttonTarget) {
          const itemElement = createCarouselItem(product, true);

          injectShowcaseWrapper
            .querySelector(".injectShowcaseSecondaryProductsWrapper__carousel")
        ...
        for (const currentProduct of currentProductsElement) {
          if (
            currentProduct.textContent
              .toLowerCase()
              .includes(product.identifier.toLowerCase())
          ) {
            product.buttonTarget = currentProduct.querySelector(
              product.selectorTriggerClick
            );

            if (!product.description) {
              product.description =
                currentProduct.querySelectorAll(
                  CTA_PRODUCTS_SELECTOR_DESCRIPTION
                )?.[1]?.textContent || "";
            }
    ...
    function reinjectComponentAfterOpenAndCloseSeats() {
      const checkIfPageIsOpen = setInterval(() => {
        ...
        const seatPageIsOpen = pagesProducts[2].classList.contains(
          "azul-page-fragment--active"
        );
    ...
    function reinjectComponentWhenSeatPageClosed() {
      const checkIfPageIsClosed = setInterval(() => {
        ...
        if (!seatPageIsOpen) {
          console.log(
            "Seat selection page is closed, reinjecting the showcase..."
          );
    ...
    function handleCarouselActions() {
      ...
      const carouselMaxScroll =
        carouselItems[carouselItems.length - 1].getBoundingClientRect().right -
        carouselRightEdge;
      const carouselMaxDotsScroll =
        (carouselMaxScroll + widthToScroll) / widthToScroll;
      const carouselMaxDotsScrollParsed = Math.ceil(carouselMaxDotsScroll);

      for (
        let dotIndex = 0;
        dotIndex < carouselMaxDotsScrollParsed;
        dotIndex++
      ) {
      ...
      const arrowLeft = document.querySelector(
        ".injectShowcaseSecondaryProductsWrapper__actions__arrow[direction='left']"
      );
      const arrowRight = document.querySelector(
        ".injectShowcaseSecondaryProductsWrapper__actions__arrow[direction='right']"
      );
      ...
      arrowLeft.addEventListener("click", () => {
        ...
        const previousDotItem =
          activeDot?.parentElement?.previousElementSibling;

        if (!previousDotItem) {
          dotsContainer.lastElementChild
            ?.querySelector(
              ".injectShowcaseSecondaryProductsWrapper__actions__dots__dot__button"
            )
            ?.click();
      ...
      arrowRight.addEventListener("click", () => {
        ...
        if (!nextDotItem) {
          dotsContainer.firstElementChild
            ?.querySelector(
              ".injectShowcaseSecondaryProductsWrapper__actions__dots__dot__button"
            )
            ?.click();
    ...
    function trackingAnalyticsEventForOthers() {
      ...
      const manageLuggageButton = document.querySelector(
        "button[aria-label='Para gerenciar as bagagens dos viajantes, selecionar']"
      );
      const manageSeatsButton = document.querySelector(
        "button[aria-label='Para gerenciar os assentos dos viajantes, selecionar']"
      );
      ...
      const moreServicesButton = document.querySelector(
        "button[aria-label='Para gerenciar mais serviços dos viajantes, selecionar']"
      );

      moreServicesButton?.addEventListener(eventToTrack, () => {
        const checkIfModalMoreServicesIsOpen = setInterval(() => {
          const addInsuranceModalButton = document.querySelector(
            "button[aria-label='Adicionar seguros']"
          );

          if (addInsuranceModalButton) {
            const addPetModalButton = document.querySelector(
              "button[aria-label='Adicionar Pet na cabine']"
            );
            const addSpecialAssistanceModalButton = document.querySelector(
              "button[aria-label='Assistência Especial']"
            );
            ...
            addSpecialAssistanceModalButton?.addEventListener(
              eventToTrack,
              () => {
