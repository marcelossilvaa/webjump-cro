(function () {
    ...
    const tariffs = {
        maisAzul: {
            name: "Mais Azul",
            class: "inject-upgrade-tariff--maisAzul",
            tariffTrigger: "Azul",
            title: "Que tal um upgrade?",
            description: "Aproveite mais benefícios e tenha uma melhor experiência.",
            buttonText: "Fazer upgrade",
            benefits: [
                "Bagagem despachada (23kg)*",
                "Antecipação de assento comum",
                "Antecipação de voo",
            ],
        },
        azulSuper: {
            name: "Azul Super",
            class: "inject-upgrade-tariff--azulSuper",
            tariffTrigger: "Mais Azul",
            title: "Que tal um upgrade?",
            description: "Aproveite mais benefícios e tenha uma melhor experiência.",
            buttonText: "Fazer upgrade",
            benefits: [
    ...
    const initializeUpgradePromo = () => {
        ...
        async function processToSuggestUpgrade(event) {
            ...
            const upgradeTariffWrapper = optedTariffWrapper.nextElementSibling;
            
            const optedTariffProperties = {
                tariffName: optedTariffWrapper.querySelector(SELECTORS.tariffName).textContent,
                tariffPrice: optedTariffWrapper.querySelector(SELECTORS.tariffPrice).textContent,
            };

            optedTariffProperties.tariffPriceNumber = parseBRLToNumber(optedTariffProperties.tariffPrice);
            optedTariffProperties.tariffPriceFormatted = formatNumberToBRL(optedTariffProperties.tariffPriceNumber);
            ...
            const promotionalTariff = Object.values(tariffs).find(t => t.tariffTrigger === optedTariffProperties.tariffName);

            if(!promotionalTariff) {
                console.log("[AT] No promotional tariff found for:", optedTariffProperties.tariffName);
                return;
            }
            
            const upgradeTariffProperties = {
                ...promotionalTariff,
                tariffPrice: upgradeTariffWrapper.querySelector(SELECTORS.tariffPrice).textContent,
            ...
            upgradeTariffProperties.tariffPriceNumber = parseBRLToNumber(upgradeTariffProperties.tariffPrice);
            upgradeTariffProperties.tariffPriceFormatted = formatNumberToBRL(upgradeTariffProperties.tariffPriceNumber);
            upgradeTariffProperties.differencePrice = upgradeTariffProperties.tariffPriceNumber - optedTariffProperties.tariffPriceNumber;
            upgradeTariffProperties.differencePriceFormatted = formatNumberToBRL(upgradeTariffProperties.differencePrice);
        ...
        function showUpgradeTariffModal(tariff) {
            ...
            return new Promise((resolve) => {
                    ...
                    <div class="inject-upgrade-tariff [tariff_class]" role="alert" aria-label="Oferta de melhoria de tarifa">
                        ...
                        <div class="inject-upgrade-tariff__cta">
                            <h3 class="inject-upgrade-tariff__name">
                                Faça um upgrade para <span class="inject-upgrade-tariff__name__highlight">[tariff_name]</span> por mais:
                            </h3>
                            <h3 class="inject-upgrade-tariff__price">
                                <span class="inject-upgrade-tariff__price__currency">R$</span>[tariff_price]<span class="inject-upgrade-tariff__price__cents">,[tariff_cents]</span>
                            </h3>
                ...
                const priceSplited = splitMoney(tariff.differencePriceFormatted);
                ...
                modal.innerHTML = modal.innerHTML.replace(/\[tariff_price\]/g, priceSplited[0]);
                modal.innerHTML = modal.innerHTML.replace(/\[tariff_cents\]/g, priceSplited[1]);
        ...
        function injectCustomStyle() {
            ...
            style.innerHTML = `
                ...
                .inject-upgrade-tariff--maisAzul .inject-upgrade-tariff__benefits__item:first-child:after {
                    content: "1x";
                }

                .inject-upgrade-tariff--azulSuper .inject-upgrade-tariff__benefits__item:first-child:after {
                    content: "2x";
                }
        ...
        function splitMoney(value = "0,00") {
            return value.split(",");
        }
        ...
        function analyticsEvent(upgraded, offerTariff) {
            ...
            const labelEvent = "AT_upgrade_tarifa_v1 " + offerTariff + (upgraded ? "--Aceita" : "--Recusada");
