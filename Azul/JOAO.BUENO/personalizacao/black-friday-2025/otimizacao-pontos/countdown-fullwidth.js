...
// - Renovação: 28/11 às 00:00:00 (automaticamente muda para FASE 2)
// - FASE 2: Conta até 28/11 às 23:59:59
//
const COUNTDOWN_END_DATE_PHASE_1 = '2025-11-27 23:59:59'; // FASE 1 termina em 27/11 às 23:59:59

const RENEWAL_DATE = '2025-11-28 00:00:00'; // Renovação em 28/11 à meia-noite

const COUNTDOWN_END_DATE_PHASE_2 = '2025-11-28 23:59:59'; // FASE 2 termina em 28/11 às 23:59:59
...
(function () {
  ...
  function getEndDate() {
    ...
    const renewalDate = createBrasiliaDate(RENEWAL_DATE);
    ...
    // Se já passou ou chegou na data de renovação, usar a FASE 2
    if (now >= renewalDate) {
      return phase2EndDate;
    } else {
      // Antes da renovação, usar a FASE 1
  ...
  // Função para encontrar o button do banner
  function findBannerButton() {
    // Buscar o container com as classes específicas
    const containers = document.querySelectorAll('.container-capsule.containerDefaultNoPadding');

    for (let i = 0; i < containers.length; i++) {
      const container = containers[i];
      // Verificar se contém um button com as imagens específicas
      const button = container.querySelector('button');
      if (button) {
        const images = button.querySelectorAll('img');
        const hasBannerImages = Array.from(images).some(
          (img) =>
            img.src.includes('banner-header-mobile-buy.png') ||
            img.src.includes('banner-header-desk-buy.png')
        );
        if (hasBannerImages) {
          return button;
        }
