...
function trackIncentive(action, linkName) {
 ...
 // 1) clique no botão do componente novo
comp.querySelector(".cta-button").addEventListener("click", function() {
  trackIncentive("novo", "cta_novo_cadastro");
});
  ...
  trackIncentive("nao_visualizado", "cta_nao_visualizado");
  return false;
}
    ...
    if (!document.getElementById(SETTINGS.componentId)) {
      trackIncentive("visualizado", "cta_visualizado");
      var cmp = createComponent();
      parent.parentNode.insertBefore(cmp, parent);
      
    
      // ─── 2) CLIQUE NO BOTÃO NATIVO "Realizar cadastro" ───
    // encontra pelo texto exato
   var span = Array.from(document.querySelectorAll('.rte-small'))
      .find(el => el.textContent.trim()==='Realizar cadastro');
