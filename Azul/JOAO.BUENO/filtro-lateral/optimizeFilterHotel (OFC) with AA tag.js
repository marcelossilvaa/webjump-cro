(function () {
  ...
  const initilizeFiltersOptimization = () => {
    ...
    if (emptyResultsElement) {
      ...
      reorganizeClasses();
      bindHotelFilterExpandAnalytics();

      // === AA helpers (event90 + eVar82) ===
      ...
      // Dispara somente "expand" ao clicar no cabeçalho do filtro (vertical - hotéis)
      function bindHotelFilterExpandAnalytics() {
        // wrapper dos filtros no layout de hotéis
        var filtersWrapper = originalContentWrapper.querySelector(
          ".styles__StyledFilterWrapper-sc-ulubtd-0"
        );
        if (!filtersWrapper || filtersWrapper.__atHotelExpandBound) return;
        filtersWrapper.__atHotelExpandBound = true;

        filtersWrapper.addEventListener(
          "click",
          function (ev) {
            // botão do cabeçalho do dropdown no layout de hotéis
            var headerBtn = ev.target.closest(
              ".styles__DropdownContainer-sc-1h37srh-13 > button, " +
                ".styles__DropdownContainer-sc-1h37srh-13 button, " +
                ".styles__DropdownContainer-sc-1h37srh-13 [role='button']"
            );
            if (!headerBtn || !filtersWrapper.contains(headerBtn)) return;

            var container = headerBtn.closest(
              ".styles__DropdownContainer-sc-1h37srh-13"
            );
            // rótulo do filtro: prioriza o texto do próprio botão
            // PEGA O NOME DO FILTRO SEM DUPLICAR
            var label = headerBtn.getAttribute("data-text"); // 1) mais estável

            if (!label) {
              // 2) apenas texto visível (ignora .hiddenSpan)
              var textWrap = headerBtn.querySelector(
                ".styles__WrapperLabelText-sc-1h37srh-5"
              );
              if (textWrap) {
                label = Array.prototype.map
                  .call(textWrap.childNodes, function (n) {
                    return n.nodeType === 3 ? n.textContent : ""; // só text nodes
                  })
                  .join("")
                  .trim();
              }
            ...
            label = normalizeText(label) || "Sem rotulo";

            // Opcional: tentar enviar só quando ABRIR (se houver aria-expanded)
            setTimeout(function () {
              var expanded = headerBtn.getAttribute("aria-expanded");
              if (expanded === "false") return; // clicou para fechar → não envia
              // "true" ou sem atributo: considera como expand
              analyticsEvent("AT_filtros_vertical_hoteis:click " + label); // event90 + eVar82
            }, 0);
          },
          true
        );
