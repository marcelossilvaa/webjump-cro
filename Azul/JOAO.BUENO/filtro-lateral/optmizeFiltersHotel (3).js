(function() {
    const checkIfDomReady = () => {
        ...
        if(!isHotelPage) {
            console.log("[AT] Page is not a hotel page, skipping filters optimization.");
            return;
        }
    ...
    const initilizeFiltersOptimization = () => {
        const originalContentWrapper = document.querySelector(".styles__SearchListWrapper-sc-oxmbkx-8");

        if (!originalContentWrapper) {
            console.log("[AT] Original content wrapper not found: .styles__SearchListWrapper-sc-oxmbkx-8");
            return;
        }
        ...
        function reorganizeClasses() {
            ...
            originalContentWrapper.classList.remove("inject-main-grid-wrapper--on-loading");

            const filtersElement = originalContentWrapper.querySelector(".styles__StyledFilterWrapper-sc-ulubtd-0");
            filtersElement?.classList.add("inject-filters-element");

            const bannerElement = originalContentWrapper.querySelector(".styles__DynamicBannerWrapper-sc-1u6m6gj-0");
            bannerElement?.classList.add("inject-banner-element");

            if(!bannerElement) {
                originalContentWrapper.classList.add("inject-main-grid-wrapper--no-banner");
            }

            const resultsElement = originalContentWrapper.querySelector(".styles__ListWrapper-sc-1oit4q5-6");
            resultsElement?.classList.add("inject-results-element");

            const paginationElement = originalContentWrapper.querySelector(".styles__PaginationWrapper-sc-oxmbkx-7");
            paginationElement?.classList.add("inject-pagination-element");
        }

        function reorganizeLoadingClasses() {
            originalContentWrapper.classList.add("inject-main-grid-wrapper--on-loading");

            const loadingElement = originalContentWrapper.querySelector(".styles__LoadingWrapper-sc-oxmbkx-2");
