...
(function () {
  ...
  class AzulSeatsBanner {
    constructor() {
      ...
      this.hasChildAndAdult = false;
      this.childAndAdultTrackingSent = false;
      this.passengersObserver = null;
      this.passengersDebounceTimer = null;
      this.hasAzulTariff = false;
    ...
    init() {
      ...
      this.checkTudoAzulTier(); // Nova verificação de tier
      this.attachButtonListener(); // Apenas adiciona o listener do botão
    ...
    // REMOVIDO: isOnFlightSelectionUrl() - não é mais necessário
    ...
    // REMOVIDO: loadStoredTariffInfo()
    // REMOVIDO: storeTariffInfo()
    // REMOVIDO: checkForAzulTariff() - substituído por attachButtonListener direto no init

    attachButtonListener() {
      // Remove listener anterior se existir
      if (this.buttonListener) {
        document.removeEventListener('click', this.buttonListener, true);
      }
      ...
      document.addEventListener('click', this.buttonListener, true);
    }
    ...
    captureCurrentTariff() {
      ...
      // Esses elementos aparecem nos cards de voo selecionados
      const promotionalElements = document.querySelectorAll('.promotional');
      
      promotionalElements.forEach((el) => {
        const text = (el.textContent || el.innerText || '').trim();
        tarifasEncontradas.push(text);
        
        // Verifica se o texto é EXATAMENTE "Azul" (não "Mais Azul", "Super Azul", "Business", etc.)
        if (text === 'Azul') {
          foundAzulOnly = true;
        }
      ...
      if (tarifasEncontradas.length === 0) {
        // Busca em elementos de tarifa selecionada
        const selectedFares = document.querySelectorAll('.fare.selected, .fare-selected, [class*="selected"] .fare-name, [class*="selected"] [class*="promotional"]');
        selectedFares.forEach((el) => {
          const text = (el.textContent || el.innerText || '').trim();
          tarifasEncontradas.push('selected: ' + text);
          if (text === 'Azul') {
            foundAzulOnly = true;
          }
          if (text === 'Business' || text.toLowerCase().includes('business')) {
    ...
    checkTudoAzulTier() {
      try {
        ...
        if (this.hasFreeSeatTier && !this.freeSeatTierTrackingSent) {
          analyticsEvent('has_free_seat_tier');
          this.freeSeatTierTrackingSent = true;
        }
    ...
    injectCardWhenReady() {
      ...
      // Verifica se há criança e adulto E tarifa Azul E se NÃO tem tier premium
      if (!this.hasChildAndAdult || !this.hasAzulTariff || this.hasFreeSeatTier) return;
