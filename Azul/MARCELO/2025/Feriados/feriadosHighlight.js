(function() {
  ...
  function isHolidayActive(item) {
      if (!item || !item.endDate) {
          return true;
      }

      return getTodayDateKey() <= item.endDate;
  }

  function getActiveHolidayItems() {
      return holidayItems.filter(function(item) {
          return isHolidayActive(item);
      });
  }

  function hasActiveHolidays() {
      return getActiveHolidayItems().length > 0;
  }
  ...
  function ensureHolidayLinks(banner) {
      ...
      const activeItems = getActiveHolidayItems();
      ...
      const existingWrapper = banner.querySelector('.at-holidays-modal-links-wrapper');
      if (existingWrapper) {
...
.at-holidays-modal-banner {
  background: linear-gradient(60.45deg, rgb(0, 97, 160) 6.27%, rgb(0, 139, 196) 96.6%) !important; !important;
...
`;
  ...
  function customizeBanner() {
      ...
      if (!hasActiveHolidays()) {
          removeBannerIfExists();
          stopObserver();
          console.log('[AT] Nenhum feriado ativo, banner removido.');
          return;
      }
  ...
  function experienceSetup() {
      ...
      if (!hasActiveHolidays()) {
          removeBannerIfExists();
          console.log('[AT] Nenhum feriado ativo, experiência não será exibida.');
          return;
      }
