(function () {
  ...
  function applyFilter(wrapper, label, useSteps = false) {
    ...
    setTimeout(() => {
      if (useSteps) {
        ...
        const current = wrapper.querySelector('.css-pdoeiw-singleValue')?.textContent.trim();
  ...
  function injectCSS() {
    ...
    section.card-list .flight-card .flight-card__container {
    ...
    section.card-list .flight-card:not([aria-label="Voo esgotado"]) .flight-card__container:hover {
  ...
  // ---------- 5) ORDENA VOOS PARA O FIM DA LISTA ----------
  function orderSoldOutFlightsToEnd() {
    const sectionWrapper = document.querySelectorAll(SELECTORS.contentWrapper);
    const flightItems = document.querySelectorAll(SELECTORS.flightItem);

    if (!sectionWrapper || !flightItems) return;
    ...
    const filteredSoldOutFlightItems = Array.from(flightItems).filter((card) => {
    ...
    filteredSoldOutFlightItems.forEach((item) => {
      item.style.order = 2;
    });
  }

  const flightItemsListObserver = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      if (mutation.addedNodes.length > 0 || mutation.removedNodes.length > 0) {
        orderSoldOutFlightsToEnd();
      }
    });
  });

  const addingFlightsObserver = () => {
    document.querySelectorAll(SELECTORS.contentWrapper).forEach((item) => {
      flightItemsListObserver.observe(item, {
        childList: true,
      });
    });
  };

  const handleClickBookingDate = () => {
    const buttonsBookingDate = document.querySelectorAll(SELECTORS.bookingDateButtons);

    buttonsBookingDate?.forEach((button) => {
      button.removeEventListener('click', setupForHandleClickBookingDate);
      button.addEventListener('click', setupForHandleClickBookingDate);
    });
  };

  const setupForHandleClickBookingDate = () => {
    const intervalReconstructionWrapper = setInterval(() => {
      const wrapperRemoved = !checkWrapperOfFlights();

      if (wrapperRemoved) {
        clearInterval(intervalReconstructionWrapper);
        const intervalCheckBookingChangeIsFinishe = setInterval(() => {
          const wrapperIsFinished = checkWrapperOfFlights();

          if (wrapperIsFinished) {
            setupForInitOrderSoldOutFlightsToEnd();
            clearInterval(intervalCheckBookingChangeIsFinishe);
          }
        }, 150);
      }
    }, 100);
  ...
  const handleFlightSelect = () => {
    const buttonsFlightSelect = document.querySelectorAll(SELECTORS.flightSelectButton);

    buttonsFlightSelect?.forEach((button) => {
      button.addEventListener('click', () => {
        setTimeout(handleClickBookingDate, 300);
      });
    });
  };

  const setupForInitOrderSoldOutFlightsToEnd = () => {
    handleFlightSelect();
    orderSoldOutFlightsToEnd();
    addingFlightsObserver();
    handleClickBookingDate();
  };

  const checkWrapperOfFlights = () => {
    const containerOfWrapper = document.querySelector(SELECTORS.contentWrapper) ? true : false;
    return containerOfWrapper;
  };
