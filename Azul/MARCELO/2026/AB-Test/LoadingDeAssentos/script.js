(function() {
    ...
    const experienceName = 'AT_SeatMapLoading';
    ...
    // A injeção e a UI só acontecem em /br/pt/home/review (seatmap).
    ...
    function onCheckoutFlowPage() {
        ...
        return CHECKOUT_URL_STEPS.some(function(step) {
            return currentUrl.indexOf(step) !== -1;
        });
    }
    ...
    if (!isDesktopDevice) {
        console.log('[AT] not a desktop device, experience seats will not be started');
        return;
    }
    ...
    const TRACKING_ACTIVITY = experienceName;
    ...
    const ENTRY_LOADING_DURATION_MS = 5000;

    let observer = null;
    let loaderObserver = null;
    let debounceTimer = null;
    let progressMonitorFrame = null;
    let nativeModalSyncPending = false;
...
<circle cx="8" cy="11.35" r="0.75" fill="#6A4B0A" />
    ...
    function isReviewStep() {
        // Pathname alvo: /br/pt/home/review (query seatmap e outras variantes ok).
        return getURLPath().indexOf(TARGET_PATH) !== -1;
    }

    function isTargetPage() {
        // UI da experiência só no seatmap dentro de /home/review.
        var href = window.location.href.toLowerCase();
        return isReviewStep() && href.indexOf(TARGET_QUERY) !== -1;
    }

    function isOneCheckoutStep() {
        return onCheckoutFlowPage();
    }
...
<path d="M1 1L17 17M17 1L1 17" stroke="#F0F0F0" stroke-width="1.5" stroke-linecap="round" />
    ...
    function scheduleNativeModalSync() {
        if (nativeModalSyncPending) {
            return;
        }
        nativeModalSyncPending = true;
        window.requestAnimationFrame(function() {
            nativeModalSyncPending = false;
    ...
    function applyProgressFromElapsed() {
        var elapsedRatio = Math.min(1, getEntryElapsedMs() / ENTRY_LOADING_DURATION_MS);
        var percent = Math.floor(elapsedRatio * 100);
        var padded = percent < 10 ? '0' + percent : String(percent);
        var text = document.getElementById('at-seatmap-loading-text');
        var fill = document.querySelector('.at-seatmap-progress-fill');

        if (text) {
            text.textContent = 'Carregando... ' + padded + '%';
        }
        if (fill) {
    ...
    function tickProgress() {
        ...
        var elapsedRatio = applyProgressFromElapsed();

        if (elapsedRatio >= 1) {
            progressMonitorFrame = null;
            return;
        }

        progressMonitorFrame = window.requestAnimationFrame(tickProgress);
    }

    function startProgressMonitor() {
        if (progressMonitorFrame) {
            return;
        }

        applyProgressFromElapsed();
        progressMonitorFrame = window.requestAnimationFrame(tickProgress);
    }

    function stopProgressMonitor() {
        if (progressMonitorFrame) {
            window.cancelAnimationFrame(progressMonitorFrame);
            progressMonitorFrame = null;
        }
    }
    ...
    function isResponsavelPage() {
        return getURLPath().indexOf(RESPONSAVEL_PATH) !== -1;
    }

    function cleanupInjectedExperience() {
        var skipOverlay = document.getElementById(SKIP_MODAL_OVERLAY_ID);
        if (skipOverlay && skipOverlay.parentNode) {
            skipOverlay.parentNode.removeChild(skipOverlay);
        }

        var skipButton = document.getElementById(SKIP_BUTTON_ID);
        if (skipButton) {
            var skipRow = skipButton.closest('.at-seatmap-skip-button-row');
            if (skipRow && skipRow.parentNode) {
                skipRow.parentNode.removeChild(skipRow);
            } else if (skipButton.parentNode) {
                skipButton.parentNode.removeChild(skipButton);
            }
        }

        var leftCard = document.getElementById(LEFT_COLUMN_WRAPPER_ID);
    ...
    function processLoader() {
        ...
        try {
            ...
            // Em etapas anteriores só observamos a SPA; não injetamos a UI do seatmap.
            // Em /responsavel apenas pré-ativamos o loading na transição para o seatmap.
            if (isResponsavelPage()) {
            ...
            if (!isTargetPage()) {
                ...
                // Review sem seatmap (bagagem/seguro) ou outras etapas: remove UI injetada.
                if (!shouldKeepModalDuringTransition() && isReviewStep() === false) {
                    cleanupInjectedExperience();
                }
