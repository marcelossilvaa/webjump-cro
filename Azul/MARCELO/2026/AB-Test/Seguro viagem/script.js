(function() {
    ...
    function experienceSetup() {
         ...
         * @param {string} title - título exibido no gatilho.
         * @param {string} content - HTML do painel.
         * @param {object} [options] - { tag, isOpen, trackingId }
         * @returns {string}
         */
        ...
        function createCoverageAccordions(context) {
            ...
            return context.groups.map((group, index) => {
                ...
                return createAccordion(group.title, `<ul class="injectedInsuranceModal__coverage">${items}</ul>`, {
                    tag: group.tag,
                    isOpen: index === 0,
                });
            }).join("");
        ...
        function createPriceBlock(context) {
            const { total, perPerson, fallback } = context.price;
            ...
            <div class="injectedInsuranceModal__price">
                ...
                <p class="injectedInsuranceModal__price__value">${fallback} <span class="injectedInsuranceModal__price__detail">por viajante</span></p>
            </div>
        `;
        }
        ...
        function addListenersToAccordions(modalElement) {
            ...
            accordions.forEach(accordion => {
                ...
                trigger.addEventListener("click", () => {
                    ...
                    if (willOpen) {
                        analyticsEvent("details_accordion_" + accordion.dataset.accordion);
                    }
                });
            });
        }
