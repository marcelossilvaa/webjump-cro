// POPUP ANTECIPACAO DE VOO - AZUL FIDELIDADE
// Variacoes: topazio_safira | diamante | unificado | auto

(function () {
  'use strict';

  const POPUP_ID = 'at-antecipacao-popup';
  const BUTTON_ID = 'at-antecipacao-floating-btn';
  const WRAPPER_ID = 'at-antecipacao-wrapper';
  const STYLE_ID = 'at-antecipacao-popup-styles';
  const FONT_FAMILY = '"Helvetica Neue", Arial, sans-serif';

  // auto = resolve pelo tier do cookie
  // topazio_safira | diamante | unificado = forca a variante (Target)
  const VARIANT = 'auto';

  const CTA_URL = 'https://www.voeazul.com.br/br/pt/sobreazul/para-a-sua-viagem/antecipacao-de-voo';

  const STORAGE_KEYS = {
    INTERACTED: 'at_antecipacao_popup_interacted_date',
    SESSION_SHOWN: 'at_antecipacao_popup_session_shown',
  };

  const TARGET_URLS = [
    'https://www.voeazul.com.br/br/pt/programa-fidelidade/niveis',
    'https://www.voeazul.com.br/br/pt/programa-fidelidade/como-subir-de-nivel',
    'https://www.voeazul.com.br/br/pt/programa-fidelidade/conheca-os-beneficios',
    'https://www.voeazul.com.br/br/pt/programa-fidelidade/conheca-o-programa',
  ];
  ...
  function analyticsEvent(eventLabel, eventType, variant) {
    ...
    const labelEvent = 'AT_AntecipacaoVoo' + suffix + '_' + eventType + ' ' + eventLabel;
  ...
  function isTargetPage() {
    ...
    return TARGET_URLS.some(function (target) {
      return url === target;
    });
  }
  ...
  function createTierGroupHTML(leftIcon, leftLabel, rightIcon, rightLabel, showNovo) {
    const novoBadge = showNovo
      ? '<span class="at-antecipacao-novo">NOVO</span>'
      : '<span class="at-antecipacao-novo at-antecipacao-novo--placeholder" aria-hidden="true"></span>';

    return (
      '<div class="at-antecipacao-tier-group">' +
      novoBadge +
      '<div class="at-antecipacao-tier-row">' +
      '<div class="at-antecipacao-tier-item">' +
      '<span class="at-antecipacao-tier-ico">' +
      leftIcon +
      '</span>' +
      '<span class="at-antecipacao-tier-label">' +
      leftLabel +
      '</span>' +
      '</div>' +
      '<span class="at-antecipacao-tier-amp">&amp;</span>' +
      '<div class="at-antecipacao-tier-item">' +
      '<span class="at-antecipacao-tier-ico">' +
      rightIcon +
      '</span>' +
  ...
  function createInfoCardHTML(textHtml) {
    return (
      '<div class="at-antecipacao-info-card">' +
      '<p class="at-antecipacao-info-title">Mais facilidade para você!</p>' +
      '<div class="at-antecipacao-info-row">' +
      '<span class="at-antecipacao-info-ico">' +
      iconClockSVG() +
      '</span>' +
      '<p class="at-antecipacao-info-text">' +
      textHtml +
      '</p>' +
      '</div>' +
      '</div>'
  ...
  function createPopupBodyHTML(variant) {
    let sections = '';
    let popupClass = 'at-antecipacao-popup';
    ...
    if (variant === 'topazio_safira') {
      ...
      sections =
        ...
        '<div class="at-antecipacao-divider"></div>' +
    ...
    return (
      ...
      variant +
      '">' +
      '<div class="at-antecipacao-arrow"></div>' +
      '<button class="at-antecipacao-close" type="button" aria-label="Fechar popup">&times;</button>' +
      '<div class="at-antecipacao-head">' +
      '<h2 class="at-antecipacao-title">Seus benefícios evoluíram</h2>' +
      '<p class="at-antecipacao-subtitle">Antecipação de voo gratuita</p>' +
      '</div>' +
      '<div class="at-antecipacao-divider"></div>' +
      sections +
      '<a class="at-antecipacao-cta" href="' +
      CTA_URL +
  ...
  function injectStyles() {
    ...
    styles.id = STYLE_ID;
    styles.textContent =
      '@keyframes atAntecipacaoPulse{0%{transform:scale(1)}50%{transform:scale(1.04)}100%{transform:scale(1)}}' +
      '#' +
      WRAPPER_ID +
      ...
      '#' +
      WRAPPER_ID +
      ' .at-antecipacao-popup{pointer-events:auto;}' +
      '#' +
      BUTTON_ID +
      '{position:fixed;right:22px;bottom:24px;width:60px;height:60px;border-radius:50%;display:none;align-items:center;justify-content:center;border:none;cursor:pointer;background:#DFF2FE;z-index:999998;filter:drop-shadow(0 4px 4px rgba(0,0,0,.25));transition:transform .24s ease,filter .24s ease;animation:atAntecipacaoPulse 2.2s infinite;}' +
      '#' +
      BUTTON_ID +
      ...
      '.at-antecipacao-popup{position:fixed;right:100px;bottom:24px;width:303px;max-width:calc(100vw - 120px);max-height:calc(100dvh - 48px);max-height:calc(100vh - 48px);overflow-x:hidden;overflow-y:auto;overscroll-behavior:contain;background:#DFF2FE;border-radius:16px;color:#041E42;padding:24px;box-sizing:border-box;z-index:999999;opacity:0;visibility:hidden;pointer-events:none;transform:translateY(12px) scale(.96);transition:opacity .28s ease,visibility .28s ease,transform .28s ease;display:flex;flex-direction:column;gap:16px;min-height:0;box-shadow:0 16px 40px rgba(4,30,66,.22);}' +
      '.at-antecipacao-popup:not(.active){overflow:hidden;max-height:0;min-height:0;padding-top:0;padding-bottom:0;border-width:0;box-shadow:none;gap:0;}' +
      '.at-antecipacao-popup.is-unificado{max-height:calc(100dvh - 40px);max-height:calc(100vh - 40px);}' +
      '.at-antecipacao-popup.is-unificado:not(.active){max-height:0;}' +
      '.at-antecipacao-popup.active{opacity:1;visibility:visible;pointer-events:auto;transform:translateY(0) scale(1);padding-top:40px;}' +
      '.at-antecipacao-arrow{position:absolute;right:6px;bottom:14px;width:48px;height:48px;background:#DFF2FE;transform:rotate(45deg);z-index:-1;border-radius:4px;pointer-events:none;}' +
      '.at-antecipacao-close{position:absolute;top:10px;right:16px;border:none;background:transparent;color:#041E42;font-size:24px;line-height:1;cursor:pointer;padding:0;}' +
      '.at-antecipacao-head{display:flex;flex-direction:column;align-items:center;gap:8px;text-align:center;}' +
      '.at-antecipacao-title{margin:0;font-size:20px;line-height:24px;font-weight:700;color:#041E42;text-align:center;width:100%;}' +
      '.at-antecipacao-subtitle{margin:0;font-size:14px;line-height:18px;font-weight:400;color:#041E42;text-align:center;width:100%;}' +
      '.at-antecipacao-divider{height:1px;background:rgba(4,30,66,.32);}' +
      '.at-antecipacao-tier-group{display:flex;flex-direction:column;align-items:center;gap:8px;width:100%;max-width:100%;}' +
      '.at-antecipacao-novo{background:#CF527A;border-radius:999px;padding:4px 10px;font-size:10px;font-weight:700;line-height:12px;letter-spacing:.3px;color:#fff;}' +
      '.at-antecipacao-novo--placeholder{visibility:hidden;}' +
      '.at-antecipacao-tier-row{display:flex;align-items:flex-end;justify-content:center;gap:8px;width:100%;max-width:100%;flex-wrap:wrap;}' +
      '.at-antecipacao-tier-item{display:flex;flex-direction:column;align-items:center;gap:6px;min-width:0;flex:1 1 72px;max-width:110px;}' +
      '.at-antecipacao-tier-ico{width:32px;height:32px;display:flex;align-items:center;justify-content:center;}' +
      '.at-antecipacao-tier-ico svg{width:32px;height:32px;display:block;}' +
      '.at-antecipacao-tier-label{font-size:10px;line-height:12px;font-weight:700;letter-spacing:.2px;text-align:center;color:#041E42;word-break:break-word;max-width:100%;}' +
      '.at-antecipacao-tier-amp{font-size:18px;line-height:1;color:#595959;padding-bottom:10px;flex-shrink:0;}' +
      '.at-antecipacao-info-card{background:rgba(4,30,66,.05);border:1px solid rgba(4,30,66,.1);border-radius:14px;padding:16px;display:flex;flex-direction:column;gap:12px;max-width:100%;}' +
      '.at-antecipacao-info-title{margin:0;font-size:14px;line-height:18px;font-weight:700;color:#041E42;}' +
      '.at-antecipacao-info-row{display:flex;gap:12px;align-items:flex-start;max-width:100%;}' +
      '.at-antecipacao-info-ico{width:32px;height:32px;flex-shrink:0;display:flex;align-items:center;justify-content:center;}' +
      '.at-antecipacao-info-ico svg{width:32px;height:32px;display:block;}' +
      '.at-antecipacao-info-text{margin:0;font-size:12px;line-height:16px;font-weight:400;color:#041E42;min-width:0;flex:1;overflow-wrap:break-word;word-break:break-word;}' +
      '.at-antecipacao-info-text strong{font-weight:700;}' +
      '.at-antecipacao-cta{margin-top:4px;height:45px;background:#008058;border-radius:8px;color:#fff;text-decoration:none;display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:500;transition:background .2s ease,transform .2s ease,box-shadow .2s ease;}' +
      '.at-antecipacao-cta:hover{background:#006E4B;transform:translateY(-1px);box-shadow:0 6px 16px rgba(0,0,0,.18);}' +
      '@media (max-width:768px){' +
      '#' +
      BUTTON_ID +
      '{right:16px;bottom:max(75px,env(safe-area-inset-bottom,0px));width:60px;height:60px;}' +
      '.at-antecipacao-popup{left:16px;right:16px;top:auto;bottom:calc(151px + env(safe-area-inset-bottom,0px));width:auto;max-width:none;height:auto;max-height:calc(100dvh - 167px - env(safe-area-inset-bottom,0px));}' +
      '.at-antecipacao-popup.is-unificado{max-height:calc(100dvh - 167px - env(safe-area-inset-bottom,0px));}' +
      '.at-antecipacao-arrow{right:30px;bottom:10px;width:24px;height:24px;}' +
      '}';
  ...
  function bindEvents() {
    ...
    const closeBtn = popup.querySelector('.at-antecipacao-close');
    const cta = popup.querySelector('.at-antecipacao-cta');
