(function () {
  ...
  const STYLE_ID = 'at-holidays-search-desktop-style';
  const MONTH_NAMES = [
    'Janeiro',
    'Fevereiro',
    'Março',
  ...
  function getMonthNameOfCalendar(monthElement) {
    ...
    const span = monthElement.querySelector('span');
  ...
  function injectCalendarHolidaysIfNeeded() {
    ...
    try {
      ...
      if (
        lastMonthsSeen[0] === (monthsProps[0] && monthsProps[0].name) &&
        lastMonthsSeen[1] === (monthsProps[1] && monthsProps[1].name) &&
        !isDateRangeMode
      ) {
        return;
      }
      ...
      lastMonthsSeen.length = 0;
      lastMonthsSeen.push(
        monthsProps[0] && monthsProps[0].name,
        monthsProps[1] && monthsProps[1].name
      );
  ...
  function initExperienceWhenReady() {
    ...
    if (!isDesktopDevice) {
      console.log('[AT] Nao e desktop, experiencia nao sera executada.');
      return;
    }
