(function () {
  const experienceName = "AT_EXPERIENCE_TARIFF_ICONS_MOBA";
  ...
  function experienceSetup() {
    ...
    const DONT_HAVE_ICON =
      '<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9 0C13.9706 0 18 4.02944 18 9C18 13.9706 13.9706 18 9 18C4.02944 18 0 13.9706 0 9C0 4.02944 4.02944 0 9 0ZM9 8.29297L6.35352 5.64648L5.64648 6.35352L8.29297 9L5.64648 11.6465L6.35352 12.3535L9 9.70703L11.6465 12.3535L12.3535 11.6465L9.70703 9L12.3535 6.35352L11.6465 5.64648L9 8.29297Z" fill="#EB001B"/></svg>';
    ...
    function init() {
      injectCustomCSS();
      syncExperienceByViewport();
      window.addEventListener("resize", syncExperienceByViewport);
    }

    function syncExperienceByViewport() {
      if (!onExperienceTargetPage()) return;

      if (!isMobileViewport()) {
        if (experienceActive) {
          console.log("[AT] Tariff Icons: Viewport desktop. Pausando observer.");
      ...
      console.log("[AT] Tariff Icons: Viewport mobile ativo.");
    ...
    function bodyObserverCallback() {
      if (!onExperienceTargetPage() || !isMobileViewport()) return;
    ...
    function getBenefitsList(fareItem) {
      ...
      const directUl = Array.from(fareItem.children).find(function (child) {
    ...
    function injectCustomIconsOnTariff(fareItem) {
      ...
      Object.keys(BENEFITS_TO_INJECT_ICONS).forEach(function (benefitKey) {
      ...
      const textLuggage = (luggageElement && luggageElement.textContent) || "";
    ...
    function getBenefitValueText(benefitElement) {
      ...
      if (!textItem) {
        const fallbackNode =
          benefitElement.children[0] &&
          benefitElement.children[0].childNodes[1] &&
          benefitElement.children[0].childNodes[1].textContent;

        return (fallbackNode && fallbackNode.trim()) || "";
      }
    ...
    function isInternacionalFlight(fareItem) {
      if (!fareItem) return false;

      const tariffTypeElement = fareItem.querySelector(SELECTORS.tariffType);
    ...
    function injectCustomCSS() {
      ...
      const benefitClass = "." + CLASSNAMES.benefitInjected;
    ...
    /**
     * Dispara evento Adobe Analytics da experiencia.
