(function () {
  ...
  const recommendationSkus = ["7906.90", "7908.90", "7870.90"];

  const recommendationSkusVertuo = ["7068.80", "7070.80", "7018.80"];

  // Níveis de presentes para Original Line (OL)
  const giftTiersOL = [
    { threshold: 70, gift: "10 Cafés Pistachio" },
    { threshold: 100, gift: "1 Kit para Drinks" },
    { threshold: 150, gift: "1 Copo para Drinks + 1 Forma de Gelo" },
    { threshold: 200, gift: "1 Bolsa Térmica" },
    { threshold: 250, gift: "1 Caneca Térmica" },
  ];

  // Níveis de presentes para Vertuo Line (VL)
  const giftTiersVL = [
    { threshold: 50, gift: "10 Cafés Pistachio" },
  ...
  const getNextGift = (totalCapsules, technology) => {
    const giftTiers = technology === "VL" ? giftTiersVL : giftTiersOL;
  ...
  const renderOffersComponent = async (container, cartItems) => {
    ...
    // Seleciona os SKUs para recomendar baseado na tecnologia
    const skusToRecommend =
      technology === "VL" ? recommendationSkusVertuo : recommendationSkus;
