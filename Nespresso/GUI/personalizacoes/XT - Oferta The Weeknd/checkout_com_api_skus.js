(async function () {
  ...
  if (window.location.pathname !== "/br/pt/checkout") {
    return;
  }
  ...
  // Verificar se o carrinho contém SKUs elegíveis
  var skusElegiveis = ["7921.90", "7077.80", "7082.80", "150331", "150343"];
  var itensCarrinho = (await window.napi.cart().read()) || [];
  var possuiSkuElegivel = itensCarrinho.some(function (item) {
    var sku = item.productId.replace("erp.br.b2c/prod/", "");
    return skusElegiveis.indexOf(sku) !== -1;
  });

  if (!possuiSkuElegivel) {
    return;
  }
  ...
  const NespressoBanner = {
    ...
    defaultConfig: {
      targetSelector: "div[id*='Shopping-Bag-Top-Banner'],#block-8831586620917",
    ...
    insertComponent: async function (config) {
      try {
        ...
        sendGAEvent("visualizou_oferta_the_weeknd");
