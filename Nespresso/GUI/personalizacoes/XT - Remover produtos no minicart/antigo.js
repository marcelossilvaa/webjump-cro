...
(function () {
  ...
  function createRemoveAllButton(hasRemovableItems) {
    ...
    // Remove wrapper existente para evitar duplicação ou se não houver itens
  ...
  async function insertRemoveButtons(cartItems) {
    ...
    // Verifica se existe pelo menos um item removível
    const hasRemovableItems = cartItems.some(
      (item) => item.unitPrice > 0 && !item.nonRemovable,
    );

    // Cria/recria (ou remove) o botão "Remover todos"
  ...
  async function handleMiniCartOpen() {
    try {
      ...
      if (cartItems && cartItems.length > 0) {
        ...
        // Se não houver itens, garante que o botão "Remover todos" seja removido
  ...
  const observer = new MutationObserver(function (mutations) {
    ...
    const miniCartOpened = mutations.some(
      (m) =>
        m.type === "childList" &&
        m.target.getElementsByClassName("MiniBasketDropdown__dropdown--is-open")
          .length > 0,
    );
