(function () {
  ...
  function setupCartSync() {
    ...
    if (bound) {
      syncCTAsWithCart(); // Checagem inicial
      return;
    }
    // Caso napi ainda não esteja pronto, tenta por até ~10s
  ...
  function addToCart(sku, quantity, button, cardId) {
    ...
    try {
      if (
        ...
        // Feedback imediato; estado final será garantido pelo sync via cart.update
        button.classList.remove("loading");
  ...
  function createCardElement(cardConfig, templateArticle) {
    ...
    if (
      ...
      htmlParts.push(
        ...
        <button type="button" class="btnCardCrossSell addCart" 
          ...
          data-sku="${variantConfig.ctaSKU}"
          data-quantity="${variantConfig.ctaQuantity}"
          data-cta-text="${variantConfig.ctaText}">
          ${variantConfig.ctaText}
        </button>`
      );
    ...
    if (ctaButton) {
      // Registra botão para sincronização com carrinho
  ...
  function processCard(cardConfig) {
    ...
    // Remover cards existentes que estão em posições incorretas (e desregistrar CTAs)
    removeCard(cardId);
  ...
  function removeCard(cardId) {
    ...
    if (cardData) {
      cardData.gridElements.forEach((element) => {
        if (element) {
          // Desregistrar CTAs desse card antes de remover do DOM
    ...
    document.querySelectorAll(`.banner-custom-` + cardId).forEach((element) => {
      // Fallback caso tenha ficado órfão
      deregisterCTAsWithin(element);
      element.remove();
    });
  ...
  function updateCards() {
    ...
    // Após atualizar/gerar cards, sincroniza CTAs com carrinho
    setupCartSync();
  }
  ...
  function initialize() {
    ...
    // Garantir verificação inicial e listener de carrinho
    setupCartSync();
