(function () {
  ...
  const giftTiersVertuo = [
    {
      threshold: 50,
      gift: "10 cafés Costa Rica",
      shortName: "Ganhe 10 cafés Costa Rica",
      image:
  ...
  // Função para identificar a tecnologia do produto
  // Função para verificar se o SKU é Master Origins
  const isMasterOrigins = (productId) => {
    // Converte para string e remove espaços para garantir comparação correta
    const cleanProductId = String(productId).trim();

    // Verifica se o productId corresponde exatamente ou contém algum dos SKUs
    return masterOriginsSKUs.some((sku) => {
      const cleanSku = sku.trim();
      // Verifica correspondência exata ou se o productId contém o SKU
      return (
        cleanProductId === cleanSku ||
        cleanProductId.includes(cleanSku) ||
        cleanSku.includes(cleanProductId)
      );
    });
  };
  ...
  const renderOffersComponent = (
    ...
    totalCapsules,
    technology = "original",
    masterOriginsCount = 0
  ) => {
    // Verifica primeiro se tem o mínimo de Master Origins
    if (masterOriginsCount < 10) {
      container.style.display = "block";

      let encouragementHtml =
        `
        <div style="background-color: #6C6B3F; padding: 12px;">
          <div style="display: flex; align-items: center; justify-content: space-between;">
            <div style="flex: 1;">
              <p style="font-size: 16px; color: #fff; margin: 0; line-height: 22px;">
                <span style="font-size: 15px;">
                  ` +
        (masterOriginsCount === 0
          ? "Adicione <strong style='font-weight: 600;background-color: #FFF;color: #4b4a2c;padding: 2px 6px;border-radius: 12px;'>10 cafés Master Origins</strong> ao seu carrinho e desbloqueie <strong style='font-weight:600;'>ofertas exclusivas!</strong>"
          : ``) +
        `
                </span>
              </p>
      ...
      container.innerHTML = encouragementHtml;
      container.style.opacity = "1";
    ...
    // Se não tem cápsulas no total (mas tem Master Origins), oculta
    if (totalCapsules === 0) {
    ...
    if (tierInfo.nextTier) {
      nextTierInfoHtml =
        ...
        (isMobile ? "4px" : "4px") +
        `;">
                <div style="display: flex; align-items: center; justify-content: space-between; background-color: #E2D6C6; padding: ` +
        ...
        <span style="font-size: 14px;color: #FFF;font-weight: 600;margin: 0;background-color: #6C6B3F;padding: 2px 6px;border-radius: 12px;">` +
  ...
  const countCapsules = async (cartItems, container) => {
    ...
    let originalCount = 0;
    let vertuoCount = 0;
    let masterOriginsCount = 0; // Contador para Master Origins
      ...
      for (const item of cartItems) {
        ...
        try {
          ...
          if (isCapsule(product)) {
            ...
            // Verifica se é Master Origins - testando com productId e também com o SKU do produto
            if (
              isMasterOrigins(item.productId) ||
              (product.sku && isMasterOrigins(product.sku))
            ) {
              masterOriginsCount += item.quantity;
            }
