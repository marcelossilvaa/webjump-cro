(function () {
  ...
  function criarCards() {
    ...
    // Busca categorias
    const categoriasPLP = document.querySelectorAll(".collection-grid");
    if (categoriasPLP.length <= 1) return;

    const reference = categoriasPLP[1].querySelector("article");
    if (!reference) return;

    // Insere CSS apenas uma vez
    inserirCSS();

    // Cria banners em cada categoria (exceto nas últimas duas)
    categoriasPLP.forEach((categoria, index) => {
      if (index >= categoriasPLP.length - 2) return;
