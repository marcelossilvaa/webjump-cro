(function () {
  ...
  function criarVitrines(products, location, tituloVitrine) {
    document
      .querySelector(location)
      .insertAdjacentHTML(
        "beforebegin",
        `<h1 class="recomendacaoTitulo">${tituloVitrine}</h1>`
      );
      ...
      window.napi
        ...
        .then(function (value) {
          try {
            ...
            // Criar HTML de intensidade apenas se for maior que zero
            let intensityHTML = "";
            if (intensity > 0) {
              intensityHTML = `
                <div class="info-box">
            ...
            // Se não houver informação de intensidade, a linha de informações pode ficar vazia
            // Verificar se devemos criar a linha de informações
            const infoRowHTML = intensityHTML
              ? `<div class="info-row">${intensityHTML}</div>`
              : ""; // Se não tiver intensidade, não mostra a linha
  ...
  function initializePersonalizedShowcases() {
    try {
      ...
      window.napi
        ...
        .then(function (value) {
          if (value) {
            lastOrders = value.orders;
            let lastFiveSKUsBought = lastOrders.slice(-5).map(function (order) {
              let productsPurchased = order.quotation.cartLines;
              return productsPurchased.map(function (product) {
                return product.item.replace("erp.br.b2c/prod/", "");
              });
            });
            if (lastFiveSKUsBought.length > 0) {
              alert("criando vitrines compras " + lastFiveSKUsBought);
              criarVitrines(
                lastFiveSKUsBought,
                ".recomendacoesPersonalizadas",
                "Reviva Seus Sabores Preferidos"
              );
            }
          }
        });
