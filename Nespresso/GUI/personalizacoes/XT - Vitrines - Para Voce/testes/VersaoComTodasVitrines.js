(function () {
  ...
  function isCapsulaProduct(sku) {
    ...
    // Padrão específico de Cápsulas: exatamente 4 dígitos + ponto + 2 dígitos
    const capsulaPattern = /^\d{4}\.\d{2}$/;

    // Padrão específico de Máquinas: contém hífen e "BR"
    const machinePattern = /.*-BR-.*/;

    // Padrão específico de Acessórios: apenas números (sem pontos ou hífens)
    const accessoryPattern = /^\d{4,6}$/; // 4 a 6 dígitos apenas números

    // Primeiro, verifica se é cápsula
    if (capsulaPattern.test(sku)) {
      return true;
    }

    // Se não é cápsula, verifica se é máquina ou acessório
    if (machinePattern.test(sku) || accessoryPattern.test(sku)) {
      return false;
    }

    // Se não corresponder a nenhum padrão conhecido, não considerar como cápsula
    console.warn(`SKU com padrão desconhecido encontrado: ${sku}`);
    return false;
  }

  // Função para analisar todas as compras e encontrar o produto mais comprado
  async function getMostPurchasedProduct() {
    try {
      ...
      ordersData.orders.forEach(function (order) {
        if (order.quotation && order.quotation.cartLines) {
          order.quotation.cartLines.forEach(function (cartLine) {
            ...
            // Filtrar apenas cápsulas usando a nova função melhorada
      ...
      console.log(
        `Análise concluída: ${capsulaProducts} cápsulas de ${totalProducts} produtos totais`
      );

      // Encontrar o SKU com maior quantidade
      ...
      console.log(
        `🏆 Produto mais comprado: ${mostPurchasedSKU} com ${highestQuantity} unidades`
      );
  ...
  // Função para criar a vitrine do produto mais comprado
      ...
      try {
        ...
        if (mostPurchasedData) {
          ...
          if (mostPurchasedShowcase && mostPurchasedShowcase.SKUs.length > 0) {
            console.log(
              "Criando vitrine do produto mais comprado:",
              mostPurchasedShowcase.SKUs[0]
            );

            // Usar a mesma função das outras vitrines
            await criarVitrines(
              mostPurchasedShowcase,
              "#recomendationForYou",
              mostPurchasedShowcase.title,
              `Você já comprou ${mostPurchasedData.quantity} unidades deste café`
            );
          }
        }
      } catch (mostPurchasedError) {
        console.error(
          "Erro ao criar vitrine do produto mais comprado:",
          mostPurchasedError
        );
      }
      try {
