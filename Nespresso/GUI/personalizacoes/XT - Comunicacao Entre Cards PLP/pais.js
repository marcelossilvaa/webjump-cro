(function () {
  ...
  const cardState = {
    ...
    // Novo: controle de eventos GA4 enviados
  ...
  function getCachedGridElements(collections) {
    ...
    if (cardState.cachedSelectors.has(cacheKey)) {
      // Verificar se elementos ainda existem no DOM
  ...
  function createCardElement(cardConfig, templateArticle) {
    ...
    if (variantConfig.ctaLink && variantConfig.ctaText) {
      htmlParts.push(
        ...
        <a href="` +
          ...
          `" class="linkCardCrossSell" data-card-id="` +
          cardConfig.id +
          `" data-cta-text="` +
    ...
    // NOVO: Event tracking para CTA button
    ...
    if (ctaLink) {
      ctaLink.addEventListener("click", (event) => {
        ...
        // Enviar evento GA4 para clique no botão
  ...
  // ——————————————————————————————————————————————————————————————————————————————
  // NOVA FUNÇÃO: TRACKING DE CARREGAMENTO DE CARDS
  // ——————————————————————————————————————————————————————————————————————————————
  ...
  function trackCardLoad(cardId) {
    ...
    // Enviar evento GA4 para carregamento da comunicação
  ...
  function processCard(cardConfig) {
    ...
    if (!targetGrids.length) {
      // Remove card se não há grids target
    ...
    // Verificar se card já existe nos grids corretos
    ...
    if (hasAllCards) {
      // Card já existe em todos os grids necessários
    ...
    // Remover cards existentes que estão em posições incorretas
    removeCard(cardId);
    ...
    // NOVO: Track o carregamento do card
  ...
  function removeCard(cardId) {
    ...
    // Também remover qualquer card órfão no DOM
    document
      .querySelectorAll(`.banner-custom-` + cardId)
      .forEach((element) => element.remove());

    // NOVO: Remover do tracking quando card é removido
  ...
  function updateCards() {
    ...
    if (filterChanged) {
      ...
      // Limpar cache de seletores quando filtro muda
      cardState.cachedSelectors.clear();
      // Remover todos os cards ativos
      Array.from(cardState.activeCards.keys()).forEach(removeCard);
      // NOVO: Limpar eventos de carregamento enviados quando filtro muda
      cardState.sentLoadEvents.clear();
    }

    // Processar cards baseado na configuração
  ...
  function createOptimizedObserver() {
    ...
    // Observer mais específico e eficiente
    mutationObserver = new MutationObserver((mutations) => {
      ...
      for (const mutation of mutations) {
        ...
        if (mutation.type === "childList") {
          // Verificar se houve mudanças em grids de coleção
      ...
      if (shouldUpdate) {
        ...
        updateTimeout = setTimeout(updateCards, 150); // Debounce aumentado
      }
    });

    // Observer mais restrito
    mutationObserver.observe(plpCardsGridElement, {
      childList: true,
      subtree: true,
      attributes: true,
      attributeFilter: ["data-filter-counter"], // Apenas este atributo
    });
  ...
  function initialize() {
    ...
    // Definir variante do produto
    cardState.productVariantKey = location.href.includes("original")
      ? "ol"
      : "vl";
