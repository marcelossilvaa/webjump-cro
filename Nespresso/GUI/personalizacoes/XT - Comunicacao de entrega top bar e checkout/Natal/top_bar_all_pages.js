if (!window.location.pathname.includes("checkout") && !window.topBarMessage) {
  (function () {
    ...
    function calcularDataNatal() {
      const anoAtual = new Date().getFullYear();
      return new Date(anoAtual, 11, 25); // 11 = dezembro (0-indexed)
    }
    ...
    function gerarMensagemEntrega() {
      ...
      if (capitalUF && capitalUF.toLowerCase() != cookieCidade.toLowerCase()) {
        diasEntrega += 3;
      }
      ...
      let dataNatal = calcularDataNatal();

      // Calcula quantos dias antes do Natal será a entrega
      let diasAntesNatal = calcularDiasEntreDatas(dataEntrega, dataNatal);
      ...
      if (diasAntesNatal > 0) {
        if (diasAntesNatal === 1) {
          mensagem =
            `Surpreenda quem você ama. Compre hoje e <strong>receba ` +
            diasAntesNatal +
            ` dia antes do Natal</strong>`;
        } else {
          mensagem =
            `Surpreenda quem você ama. Compre hoje e <strong>receba ` +
            diasAntesNatal +
            ` dias antes do Natal</strong>`;
        }
      } else if (diasAntesNatal === 0) {
        mensagem =
          "Surpreenda quem você ama. Compre hoje e <strong>receba no Natal</strong>";
      } else {
        // Se a entrega for depois do Natal, não mostra a mensagem
        return null;
      }
    ...
    function setMessagesTopBar() {
      if (!document.querySelector(".mensagemEntregaTopBar")) {
        ...
        let mensagem = dadosEntrega.mensagem;
        ...
        $(document).ready(function () {
          ...
          let CSS = `
            <style>
              #topMessageBanner, #topMessageBannerMob{
          ...
          gtmDataObject.push({
            ...
            local_event_label:
              "xt_adobe_target_mensagem_frete_" + dadosEntrega.uf, //free to fill field, please use lower case
          });
        });
      }
    }
