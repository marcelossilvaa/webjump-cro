...
function setCookieLocalizacao(cidade, uf) {
  ...
  document.cookie =
    "ufAB=" + uf + "; path=/br/pt; expires=" + expirationDate.toUTCString();
}

function getCookieAB(cookieName) {
  let cookies = document.cookie;
  let cookiesArray = cookies.split(";");
  let cookieValue = cookiesArray.find((cookie) => {
    return cookie.includes(cookieName);
  });
  if (cookieValue) {
    return cookieValue.trim().replace(cookieName + "=", "");
  } else {
    return false;
  }
}
...
function addCountdownContainer(containerAppend, containerFrete) {
  let css = `
    <style>
      #countdown, #countdown *{
      ...
      #countdown .dataEntregaContainer {
        display: flex;
        flex-direction: column;
        align-items: center;
      }
  ...
  document
    ...
    .insertAdjacentElement("beforeend", containerFrete);

  if (divGeo) {
    document.querySelector("#pdp-freight-component").style.display = "none";
    document
      .querySelector(".cidadeEntrega")
      .addEventListener("click", function () {
        document.querySelector("#pdp-freight-component").style.display =
          "block";
      });
  }
  document
    .querySelector("#pdp-freight-component button[type='submit']")
    .addEventListener("click", function () {
      setTimeout(function () {
        searchAddressAndSetCookie();
        let divGeo = obterDataEntregaCompleta();
        if (divGeo) {
          document.querySelector("#countdown .dataEntregaContainer").innerHTML =
            divGeo;
          document.querySelector("#pdp-freight-component").style.display =
            "none";
