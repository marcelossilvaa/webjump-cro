...
(function () {
  "use strict";
  if (window.newLoginDropdownBenefits) {
    return;
  }
  window.newLoginDropdownBenefits = true;
  ...
  function createBenefitsSection() {
    ...
    if (!dropdownFath || !dropdownContainer) {
      ...
      let interval = setInterval(() => {
        ...
        let currentDropdownFath = document.querySelector(
          ".LoginDropdown__dropdown"
        );
        let currentDropdownContainer = document.querySelector(
        ...
        if (tentativas >= maxTentativas) {
          clearInterval(interval);
          console.log(
            "Timeout: dropdown não encontrado após",
            maxTentativas,
            "tentativas"
          );
        }
  ...
  function newFunction(dropdownFath, dropdownContainer) {
    ...
    const existingBenefits = dropdownContainer.querySelector(
      ".LoginDropdown__benefits"
    );
    if (existingBenefits) {
    ...
    let dropdownRegister = document.querySelector(
    ...
    const registerButton = dropdownContainer.querySelector(
    ...
    if (registerButton && registerButton.parentNode === dropdownContainer) {
      ...
      const spacer = dropdownContainer.querySelector(
