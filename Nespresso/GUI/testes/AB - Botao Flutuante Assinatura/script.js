(() => {
  ...
  if (!window.location.href.includes("standing-orders#/orders/create/new")) {
    return; // Se não contém, sai da função sem executar nada
  }
  const css = `
    .floating-footer {
      position: fixed;
  ...
  const updateFreteMessage = () => {
    ...
    footerComponent.style.display =
      totalCafes > 0 && !checkStepFooterVisibility() ? "flex" : "none";
  };

  // 6) Atualiza total estimado
  const updateTotalPrice = () => {
    const sourceTotal = document.querySelector(
      "[class*='QuotationBlock__line--finalPrice'] span[class*='Price--green']"
    );
    if (sourceTotal) {
