...
(function () {
  ...
  // Insere CSS no head para ocultar Kit Degustação quando nb-sku-machine tiver id="4292-BRB"
  (function () {
    const style = document.createElement("style");
    style.textContent = `
      nb-sku-machine[id="4292-BRB"] .benefits-icons-container > div:nth-child(2), nb-sku-machine[id="4292-BRA"] .benefits-icons-container > div:nth-child(2) {
  ...
  function createBenefitsIcons() {
    ...
    let elements = Array.from(document.querySelectorAll("p.cb-text")).filter(
      (p) =>
        p.textContent.includes("Em até") && p.textContent.includes("sem juros")
    );
    ...
    if (elements.length === 0) {
      elements = Array.from(document.querySelectorAll("p")).filter(
        (p) =>
          p.textContent.includes("Em até") &&
          p.textContent.includes("sem juros")
      );
    ...
    if (elements.length === 0) {
      ...
      console.log(
        "Elementos de parcelamento não encontrados, usando caixa de desconto"
      );
    }
    ...
    if (elements.length === 0) {
      elements = Array.from(document.querySelectorAll("span, div")).filter(
        (el) =>
          el.textContent.includes("Em até") &&
          el.textContent.includes("sem juros")
      );
    ...
    elements.forEach((element, index) => {
      ...
      if (
        nextSibling &&
        nextSibling.classList.contains("benefits-icons-container")
      ) {
        console.log(
          `Card ${index + 1} já possui benefits-icons-container como irmão`
        );
        return;
      }
      ...
      if (
        parentElement &&
        parentElement.querySelector(".benefits-icons-container")
      ) {
        console.log(
          `Card ${index + 1} já possui benefits-icons-container no elemento pai`
        );
        return;
      }
      ...
      if (nearbyContainer) {
        console.log(
          `Card ${index + 1} já possui benefits-icons-container próximo`
        );
        return;
      }
      ...
      const hasDiscountBox = element.textContent.includes(
        "Ganhe até R$150 de desconto em cafés"
      );
      ...
      if (hasDiscountBox) {
        ...
        // Para elementos de parcelamento, procura pelo elemento nb-cta no card pai
        const parentCard =
          element.closest("nb-sku-machine") ||
          element.closest(
            '[class*="card"], [class*="product"], [class*="item"]'
        ...
        // Procura pelo elemento nb-cta no card pai
        const nbCtaElement = parentCard?.querySelector("nb-cta");

        if (nbCtaElement) {
          // Insere antes do elemento nb-cta
          const parentContainer = nbCtaElement.parentElement;
          parentContainer.insertBefore(iconsContainer, nbCtaElement);
          console.log(
            `Card ${index + 1} - Componente adicionado antes do nb-cta!`
          );
        } else {
          // Fallback: se não encontrar nb-cta, adiciona após o elemento atual
          ...
          console.log(
            `Card ${
              index + 1
            } - nb-cta não encontrado, adicionado após elemento de parcelamento!`
          );
        }

        console.log(`Elemento pai:`, parentCard?.className);
      }
    });
          ...
          const hasRelevantNodes = addedNodes.some(
            (node) =>
              ...
              (node.querySelector?.("p") ||
                ...
                (node.textContent?.includes("Em até") &&
                  node.textContent?.includes("sem juros")))
          );
