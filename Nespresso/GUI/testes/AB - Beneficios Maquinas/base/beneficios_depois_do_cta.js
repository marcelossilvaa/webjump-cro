...
(function () {
  ...
  // Executa apenas nas páginas de máquinas Original e Vertuo
  const path = window.location.pathname;
  if (
    !path.includes("/br/pt/order/machines/original") &&
    !path.includes("/br/pt/order/machines/vertuo")
  ) {
    return;
  }
  ...
  function createBenefitsIcons() {
    // Busca todos os nb-cta dentro de nb-sku-machine
    const skuMachines = document.querySelectorAll("nb-sku-machine");

    skuMachines.forEach((skuMachine, index) => {
      const nbCtaElement = skuMachine.querySelector("nb-cta");
      if (!nbCtaElement) return;

      // Verifica se já existe um container de benefícios neste card
      if (skuMachine.querySelector(".benefits-icons-container")) return;
      ...
      // Insere depois do elemento nb-cta
      nbCtaElement.after(iconsContainer);
    });
  }

  // Polling: aguarda os cards existirem no DOM antes de aplicar
  function waitForCards() {
    var attempts = 0;
    var maxAttempts = 50; // ~10 segundos (50 x 200ms)

    var interval = setInterval(function () {
      attempts++;
      var cards = document.querySelectorAll("nb-sku-machine nb-cta");
