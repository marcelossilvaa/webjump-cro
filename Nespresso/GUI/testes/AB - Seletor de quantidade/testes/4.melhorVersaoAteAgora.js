(function () {
  ...
  class CartOperationQueue {
    constructor() {
      ...
      this.minInterval = 300; // Intervalo inicial
      ...
      return new Promise((resolve, reject) => {
        ...
        const timeoutPromise = new Promise((_, timeoutReject) => {
          setTimeout(() => {
            timeoutReject(
              new Error(
                `Timeout: Operação para ${productSku} demorou mais que ${this.operationTimeout}ms`
              )
            );
    ...
    try {
      ...
      while (!success && cartStateManager.shouldRetry(productSku, lastError)) {
        try {
          ...
          const timeoutPromise = new Promise((_, reject) => {
            setTimeout(() => {
              reject(
                new Error(`Timeout na tentativa ${attempt} para ${productSku}`)
              );
      ...
      if (!success) {
        ...
        const errorMessage = isTimeout
          ...
          : `Falha após ${cartStateManager.maxRetries} tentativas: ${lastError?.message}`;
  ...
  async function handleCartUpdate() {
    try {
      ...
      selectorsMap.forEach((selectorData, sku) => {
        // ✅ SOLUÇÃO 1: Ignorar produtos com operações pendentes
        if (
