(function () {
  ...
  // Sistema de fila para operações de carrinho
  class CartOperationQueue {
    constructor() {
      this.queue = [];
      this.processing = false;
      this.lastOperation = 0;
      this.minInterval = 300; // Intervalo mínimo entre operações
    }
    ...
    async addOperation(productSku, operation) {
      return new Promise((resolve, reject) => {
        this.queue.push({
          ...
          reject,
          timestamp: Date.now(),
          isRetry: false, // Flag para identificar se é uma operação de retry
        });
    ...
    async processQueue() {
      ...
      console.log(
        `Processando fila de carrinho: ${this.queue.length} operações`
      );

      while (this.queue.length > 0) {
        const { productSku, operation, resolve, reject } = this.queue.shift();

        try {
          // Garantir intervalo mínimo entre operações
          const timeSinceLastOp = Date.now() - this.lastOperation;
          if (timeSinceLastOp < this.minInterval) {
            await new Promise((res) =>
              setTimeout(res, this.minInterval - timeSinceLastOp)
            );
          }

          console.log(`Executando operação para SKU: ${productSku}`);
          const result = await operation();
          this.lastOperation = Date.now();
          resolve(result);

          // Pequena pausa para permitir que a API processe
          await new Promise((res) => setTimeout(res, 50));
        } catch (error) {
          console.error(
            `Erro na operação de carrinho para ${productSku}:`,
            error
          );
          reject(error);
        }
      ...
      console.log("Fila de carrinho processada completamente");
      this.processing = false;
    }
    ...
    cancelOperations(productSku) {
      ...
      if (canceledCount > this.queue.length) {
        console.log(
          `Canceladas ${
            canceledCount - this.queue.length
          } operações para ${productSku}`
        );
      }
    }
  ...
  // Sistema de controle de estado e retry
  class CartStateManager {
    ...
    getRetryDelay(productSku) {
      const attempts = this.retryAttempts.get(productSku) || 0;
      return Math.min(this.retryDelay * Math.pow(2, attempts), 5000);
    }
  ...
  // Função para verificar se a operação foi bem-sucedida
  async function verifyCartOperation(
    ...
    try {
      ...
      if (expectedQuantity === 0) {
        ...
        return {
          success,
          reason: success ? "Produto removido" : "Produto ainda no carrinho",
        };
      } else {
        if (cartItem) {
          ...
          return { success, actualDisplayUnits, expectedQuantity };
        } else {
          return {
            success: false,
        ...
        if (cartItem) {
          ...
          return {
            ...
            reason: success
              ...
              : `Quantidade esperada: ${expectedQuantity}, encontrada: ${actualDisplayUnits}`,
...
<path d="M6.51667 7.57578H0V5.88411H6.51667L6.5 0H8.38333V5.88411H14.5V7.57578H8.38333V14H6.51667V7.57578Z" fill="#347259"/>
    ...
    decreaseBtn.addEventListener("click", function (e) {
      ...
      if (cartStateManager.isOperationInProgress(productSku)) {
        console.log(
          `Operação em andamento para ${productSku}, ignorando clique de diminuir`
        );
        return;
      }
    ...
    increaseBtn.addEventListener("click", function (e) {
      ...
      if (cartStateManager.isOperationInProgress(productSku)) {
        console.log(
          `Operação em andamento para ${productSku}, ignorando clique de aumentar`
        );
        return;
      }
    ...
    function addToCart(isDirectAdd = false) {
      ...
      if (cartStateManager.isOperationInProgress(productSku)) {
        console.log(
          `⏳ Operação em andamento para ${productSku}, ignorando...`
        );
        return Promise.resolve();
      }

      // Cancelar operações pendentes na fila
      if (
        cartQueue.hasQueuedOperations &&
        cartQueue.hasQueuedOperations(productSku)
      ) {
        cartQueue.cancelOperations(productSku);
      }

      if (isDirectAdd) {
        return cartQueue.addOperation(productSku, () =>
          executeAddToCartWithRetry(1, productSku, productConfig)
        );
      }
      ...
      addToCartTimeout = setTimeout(() => {
        cartQueue
          .addOperation(productSku, () =>
            executeAddToCartWithRetry(quantity, productSku, productConfig)
          )
          .catch((error) => {
            console.error(`💥 Erro final para ${productSku}:`, error);

            // Reverter UI em caso de falha total
            const lastKnownState =
              cartStateManager.getLastKnownState(productSku);
            if (lastKnownState) {
              console.log(`🔄 Revertendo UI para ${productSku}`);
    ...
    async function executeAddToCartWithRetry(
      ...
      if (cartStateManager.isOperationInProgress(productSku)) {
        console.log(
          `Operação já em andamento para ${productSku}, ignorando...`
        );
        return Promise.resolve();
      }
      ...
      console.log(
        `Iniciando operação para ${productSku}: ${displayQuantity} unidades (${capsulesQuantity} cápsulas)`
      );
      ...
      try {
        ...
        while (
          ...
          try {
            ...
            if (window.CartManager && window.CartManager.updateItem) {
              console.log(
                `Tentativa ${attempt}: Executando CartManager.updateItem para ${productSku}`
              );
              ...
              console.log(
                `Tentativa ${attempt}: Executando fallback para ${productSku}`
              );
              await executeOriginalButtonFallback(displayQuantity, productSku);
            }
            ...
            if (verificationResult.success) {
              ...
              console.log(
                `✓ Operação bem-sucedida para ${productSku}: ${verificationResult.reason}`
              );
            } else {
              throw new Error(
            ...
            lastError = error;
            console.warn(`Tentativa falhou para ${productSku}:`, error.message);

            // Aguardar antes da próxima tentativa (backoff exponencial)
            if (cartStateManager.shouldRetry(productSku, error)) {
              const delay = cartStateManager.getRetryDelay(productSku);
              console.log(
                `Aguardando ${delay}ms antes da próxima tentativa para ${productSku}`
              );
        ...
        if (!success) {
          const finalError = new Error(
            `Falha ao processar ${productSku} após ${cartStateManager.maxRetries} tentativas: ${lastError?.message}`
          );
    ...
    function executeOriginalButtonFallback(displayQuantity, productSku) {
      ...
      if (!productElement) {
        console.warn(
          `Elemento do produto não encontrado para SKU: ${productSku}`
        );
      ...
      if (
        ...
        console.log(
          `Executando fallback para ${productSku}: ${displayQuantity} cliques`
        );
        ...
        console.warn(`Botão original não encontrado para ${productSku}`);
