...
<path d="M6.51667 7.57578H0V5.88411H6.51667L6.5 0H8.38333V5.88411H14.5V7.57578H8.38333V14H6.51667V7.57578Z" fill="#347259"/>
    ...
    decreaseBtn.addEventListener("click", function (e) {
      ...
      if (quantity > 0) {
        ...
        if (quantity > 0) {
          ...
          // Para remover completamente do carrinho
          executeAddToCart(0, productSku, productConfig)
            .then(() => {
              console.log(`Produto ${productSku} removido do carrinho`);
            })
            .catch((error) => {
              console.error(`Erro ao remover produto ${productSku}:`, error);
            });
        }
      }
    });
    ...
    function addToCart(isDirectAdd = false) {
      // Cancelar operações pendentes para este produto se necessário
      if (cartQueue.hasQueuedOperations(productSku)) {
        console.log(`Cancelando operações pendentes para ${productSku}`);
        cartQueue.cancelOperations(productSku);
      }

      if (isDirectAdd) {
        // Execução imediata para cliques diretos no botão
        return executeAddToCart(1, productSku, productConfig);
      }

      // Para operações de incremento/decremento, usar um pequeno delay
      // mas cancelar o timeout anterior se existir
      if (addToCartTimeout) {
        clearTimeout(addToCartTimeout);
      }

      addToCartTimeout = setTimeout(() => {
        executeAddToCart(quantity, productSku, productConfig)
          .then(() => {
            console.log(
              `Produto ${productSku} adicionado com sucesso: ${quantity} unidades`
            );
          })
          .catch((error) => {
            console.error(`Erro ao adicionar produto ${productSku}:`, error);
          });
      }, 300); // Reduzido de 500ms para 300ms
    }

    // Função que executa a adição real ao carrinho
    function executeAddToCart(displayQuantity, productSku, productConfig) {
      ...
      console.log(
        `Adicionando ${displayQuantity} unidades (${capsulesQuantity} cápsulas) do produto ${productSku}`
      );

      // Adicionar operação à fila
      return cartQueue.addOperation(productSku, async () => {
        try {
          if (window.CartManager && window.CartManager.updateItem) {
            const result = await window.CartManager.updateItem(
              productSku,
              capsulesQuantity,
              null,
              null,
            ...
            console.log(
              `CartManager.updateItem executado para ${productSku}:`,
              result
            );
            return result;
          } else {
            console.log(
              `CartManager não disponível, usando fallback para ${productSku}`
            );
            return executeOriginalButtonFallback(displayQuantity, productSku);
          }
        } catch (error) {
          console.error(`Erro ao executar operação para ${productSku}:`, error);
          throw error;
        }
      });
    }
