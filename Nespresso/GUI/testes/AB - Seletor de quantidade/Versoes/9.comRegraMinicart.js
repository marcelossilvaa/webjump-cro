(function () {
  ...
  async function getCartQuantityFromAPI(productSku) {
    try {
      ...
      if (cartItem) {
        ...
        return Math.floor(cartItem.quantity / 10);
      }
  ...
  async function handleCartUpdate() {
    ...
    try {
      ...
      if (cartData && Array.isArray(cartData)) {
        cartData.forEach((item) => {
          if (!item.nonRemovable && item.productId) {
            ...
            if (match) {
              const sku = match[1];
              const boxes = Math.floor(item.quantity / 10);
              cartSkuMap.set(sku, boxes);
            }
          }
        });
      }
      ...
      selectorsMap.forEach((selectorData, sku) => {
        ...
        const currentBoxes = cartSkuMap.get(sku) || 0;

        console.log(`Atualizando SKU ${sku}: ${currentBoxes} caixas`);
        ...
        if (currentBoxes > 0) {
          ...
          quantityDisplay.textContent = currentBoxes;
          quantityDisplay.classList.remove("zero");
          decreaseBtn.disabled = false;

          // IMPORTANTE: Atualizar a quantidade interna através da função
          // Isso garante que a variável local 'quantity' seja atualizada
          updateQuantityInternal(currentBoxes);
        } else {
  ...
  async function createQuantitySelector(productElement) {
    ...
    let currentBoxesInCart = await getCartQuantityFromAPI(productSku);

    // Se a API não retornou resultado, usar o método DOM como fallback
    if (currentBoxesInCart === 0) {
      currentBoxesInCart = getCurrentCartQuantity(productElement);
    }
...
<path d="M6.51667 7.57578H0V5.88411H6.51667L6.5 0H8.38333V5.88411H14.5V7.57578H8.38333V14H6.51667V7.57578Z" fill="#347259"/>
    ...
    // Registrar o seletor no mapa global
    const selectorData = {
      selector,
      quantityDisplay,
      decreaseBtn,
      ...
      addToCartBtn,
      updateQuantity,
      updateQuantityInternal, // Adicionar a função de atualização interna
      quantity,
    };
    ...
    function executeAddToCart(boxQuantity) {
      const capsulesQuantity = boxQuantity * 10;
      ...
      selectorData.quantity = boxQuantity;
