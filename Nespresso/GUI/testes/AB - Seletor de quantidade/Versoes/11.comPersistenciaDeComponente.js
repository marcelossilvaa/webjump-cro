(function () {
  ...
  // Mapa global para rastrear os seletores por SKU
  ...
  function getProductConfig(productElement) {
    ...
    // Para cafés, buscar a quantidade na label de cápsulas
    const capsuleLabel = productElement.querySelector('div[class*="defaultCapsuleLabel"]');
  ...
  // Função para obter a quantidade atual do carrinho (fallback para o DOM)
  ...
  // Função global para atualizar todos os seletores quando o carrinho mudar
  async function handleCartUpdate() {
    ...
    try {
      ...
      selectorsMap.forEach((selectorData, sku) => {
        ...
        console.log(`Atualizando SKU ${sku}: ${cartCapsules} cápsulas = ${currentDisplayUnits} ${productConfig.displayUnit}s`);
  ...
  async function createQuantitySelector(productElement) {
    ...
    console.log(`📝 Produto ${productSku}: ${productConfig.capsulesPerBox} ${productConfig.conversionNeeded ? 'cápsulas por caixa' : 'unidade(s)'}`);

    // Verificar quantidade atual no carrinho via API primeiro, com fallback para DOM
    let currentUnitsInCart = await getCartQuantityFromAPI(productSku, productConfig);

    // Se a API não retornou resultado, usar o método DOM como fallback
    if (currentUnitsInCart === 0) {
      currentUnitsInCart = getCurrentCartQuantity(productElement, productConfig);
    }
...
<path d="M6.51667 7.57578H0V5.88411H6.51667L6.5 0H8.38333V5.88411H14.5V7.57578H8.38333V14H6.51667V7.57578Z" fill="#347259"/>
    ...
    // Registrar o seletor no mapa global com a configuração do produto
    const selectorData = {
      selector,
      quantityDisplay,
      decreaseBtn,
    ...
    // Função para observar mudanças na quantidade do carrinho (fallback)
      ...
      const quantityObserver = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
          if (
            ...
            const newQuantity = getCurrentCartQuantity(productElement, productConfig);
 ...
 function waitForGrid() {
  ...
  if (grid) {
        ...
        if (mutation.type === "childList") {
          ...
          mutation.addedNodes.forEach(function (node) {
            if (node.nodeType === 1) { // É um elemento
              if (node.matches("article[data-product-short-sku][aria-label]")) {
          ...
          if (target.matches && target.matches("article[data-product-short-sku][aria-label]")) {
    ...
    const periodicCheck = setInterval(() => {
      const currentProductCount = grid.querySelectorAll("article[data-product-short-sku][aria-label]").length;
