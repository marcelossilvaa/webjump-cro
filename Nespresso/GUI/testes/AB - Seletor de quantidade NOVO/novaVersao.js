(function () {
  ...
  /**
   * Espera até que as APIs globais da Nespresso estejam disponíveis na window.
   * @param {number} timeout - Tempo máximo de espera em milissegundos.
   * @returns {Promise<void>} - Resolve quando as APIs estão prontas, ou rejeita se o timeout for atingido.
   */
  function waitForAPIs(timeout = 10000) {
    // Espera por até 10 segundos
    return new Promise((resolve, reject) => {
      const startTime = Date.now();
      const interval = setInterval(() => {
        // A condição que estamos esperando - CartManager é obrigatório
        const apisAreReady =
          window.napi &&
          typeof window.napi.cart === "function" &&
          typeof window.napi.data === "function" &&
          window.CartManager &&
        ...
        if (apisAreReady) {
          ...
          reject(
            new Error(
              "Timeout: As APIs da Nespresso não foram inicializadas a tempo."
            )
          );
        }
  ...
  /**
   * Verifica se as APIs essenciais do carrinho estão presentes e funcionais.
   * Faz uma chamada de leitura real para garantir que a sessão do usuário
   * e a conexão com a API estão saudáveis.
   * @returns {Promise<boolean>} - Retorna true se tudo estiver OK, false caso contrário.
   */
  async function healthCheck() {
    try {
      await waitForAPIs();

      // Tenta fazer uma chamada de leitura inofensiva para validar a API
      await window.napi.cart().read();

      // Verificar se CartManager está realmente funcional
      if (
        !window.CartManager ||
        typeof window.CartManager.updateItem !== "function"
      ) {
        console.warn(
          "CartManager não está disponível - experiência não será ativada"
        );
        return false;
      }

      // Se chegou até aqui, a API está funcional.
      return true;
    } catch (error) {
      // Se qualquer verificação falhar, loga um aviso para depuração
      console.warn("Health check falhou:", error.message);
      return false;
    }
  }
  //FUNÇÃO PRINCIPAL ASSÍNCRONA PARA CONTROLAR A EXECUÇÃO
  async function initializeExperiment() {
    // Roda o Health Check antes de fazer qualquer outra coisa
    const isCompatible = await healthCheck();

    // ADICIONAR A REGRA DE SEGURANÇA (GUARD CLAUSE)
    // Se o ambiente não for compatível, aborta a execução do script.
    if (!isCompatible) {
      console.warn("A experiência de compra original será mantida.");
      sendGAEvent("seletor_de_quantidade_nao_ativado");
      return;
    }
    // --- SE A VERIFICAÇÃO PASSOU, O RESTO DO CÓDIGO ORIGINAL EXECUTA NORMALMENTE ---
    sendGAEvent("seletor_de_quantidade_ativado");
    ...
    class CartStateManager {
      ...
      updateLastKnownState(productSku, quantity) {
        this.lastKnownState.set(productSku, {
          quantity,
          timestamp: Date.now(),
        });
      }
      ...
      cleanupStuckOperations() {
        ...
        for (const [
          productSku,
          startTime,
        ] of this.operationStartTime.entries()) {
    ...
    // Função para adição manual ao carrinho usando componentes originais
    async function executeManualAddToCart(
      displayQuantity,
      productSku,
      productConfig,
      productElement
      ...
      return new Promise((resolve, reject) => {
        try {
          // Encontrar o botão original de adicionar ao carrinho
          ...
          if (!originalAddButton) {
            reject(
              new Error(
                "Botão original de adicionar ao carrinho não encontrado"
              )
            );
            return;
          }

          // Converter para quantidade de cápsulas se necessário
          ...
          // Função para simular clique e preenchimento
          async function performManualAdd() {
            try {
              // 1. Clicar no botão original para abrir o container
              originalAddButton.click();

              // 2. Aguardar o container aparecer e encontrar o input
              const quantityInput = await waitForElement(
                "input#ta-quantity-selector__custom-field"
              );

              // 3. Limpar e preencher o input com a quantidade desejada
              quantityInput.value = "";
              quantityInput.focus();

              // Simular digitação para garantir que eventos sejam disparados
              const inputEvent = new Event("input", { bubbles: true });
              const changeEvent = new Event("change", { bubbles: true });

              quantityInput.value = capsulesQuantity.toString();
              quantityInput.dispatchEvent(inputEvent);
              quantityInput.dispatchEvent(changeEvent);

              // 4. Aguardar um pouco para garantir que o valor foi processado
              await new Promise((resolve) => setTimeout(resolve, 200));

              // 5. Encontrar e clicar no botão OK
              const okButton = await waitForElement(
                "button#ta-quantity-selector__custom-ok"
              );
              okButton.click();

              // 6. Aguardar um pouco para a operação ser processada
              await new Promise((resolve) => setTimeout(resolve, 500));

              resolve({ success: true, quantity: capsulesQuantity });
            } catch (error) {
              reject(error);
            }
          }

          // Executar a adição manual
          performManualAdd();
        } catch (error) {
          reject(error);
        }
    ...
    // Função com retry e verificação COM TIMEOUT - ADAPTADA PARA USO MANUAL
    async function executeAddToCartWithRetry(
      ...
      try {
        ...
        while (
          !success &&
          cartStateManager.shouldRetry(productSku, lastError)
        ) {
          try {
            ...
            if (cartStateManager.isOperationTimedOut(productSku)) {
              throw new Error(
                `Operação para ` +
                  productSku +
                  ` excedeu tempo máximo permitido`
              );
            }

            const attempt = cartStateManager.incrementRetry(productSku);

            // Executar a operação manual com timeout individual
            const operationPromise = executeManualAddToCart(
              displayQuantity,
              productSku,
              productConfig,
              productElement
            ...
            const timeoutPromise = new Promise((_, reject) => {
              ...
              }, 15000); // 15 segundos para operação manual
            });

            await Promise.race([operationPromise, timeoutPromise]);

            // Aguardar um pouco mais para a API atualizar
            ...
            if (verificationResult.success) {
              success = true;
              cartStateManager.updateLastKnownState(
                productSku,
                displayQuantity
              );
            } else {
              throw new Error(
                `Verificação falhou: ` + verificationResult.reason
              );
            }
          } catch (error) {
            lastError = error;
    ...
    function getProductConfig(productElement) {
      ...
      const capsuleLabelKey =
        `capsule_label_` +
        productElement.getAttribute("data-product-short-sku");
    ...
    function createModal(cardId) {
        ...
        <div class="nespresso-welcome-offer-modal-container">
          ...
          <div class="nespresso-welcome-offer-modal-content">
            <div class="nespresso-welcome-offer-modal-termos">
              <p>SUA <strong>ASSINATURA DE CAFÉS</strong><br>EM 3 PASSOS SIMPLES</p>
              <ol>
                <li><strong class='title-modal'>1. Selecione seus cafés</strong><br>Escolha seus cafés favoritos para receber em casa automaticamente.<br>Garanta <strong>10% OFF</strong> e <strong>FRETE GRÁTIS</strong> em pedidos a partir de 30 cápsulas.</li>
                <li><strong class='title-modal'>2. Defina a frequência</strong><br>Selecione a frequência que deseja receber, defina o endereço, modo de entrega e pagamento mais adequados para você.</li>
                <li><strong class='title-modal'>3. Aproveite seu Nespresso</strong><br>Seu pedido chegará no endereço e período escolhidos automaticamente. Você pode alterar os cafés e quantidade de cápsulas sempre que quiser.</li>
              </ol>
      ...
      modalElement
        .querySelector("a.dp-OAC-cta")
        .addEventListener("click", function () {
          sendGAEvent("click_modal_assinatura_comece_agora");
        });
      return modalId;
    }
    ...
    function createSignatureCopy(productElement) {
      ...
      if (
        ariaLabel &&
        (ariaLabel.includes("Kit") || ariaLabel.includes("KIT"))
      ) {
        return;
      }
      if (
        ...
        setTimeout(() => {
          ...
          if (signatureElement) {
            signatureElement.addEventListener("click", function (e) {
              e.preventDefault();
              e.stopPropagation();
              sendGAEvent("click_abrir_modal_assinatura_plp");
              // Abrir modal
    ...
    //função para executar operação real - ADAPTADA PARA USO MANUAL
  ...
  <path d="M6.51667 7.57578H0V5.88411H6.51667L6.5 0H8.38333V5.88411H14.5V7.57578H8.38333V14H6.51667V7.57578Z" fill="#347259"/>
      ...
      const selector = purchaseSection.querySelector(
        ".custom-quantity-selector"
      );
      ...
      // função addToCart com debouncing por produto
      function addToCart(isDirectAdd = false) {
        ...
        if (isDirectAdd) {
          ...
          return executeOperation(1, productSku, productConfig, productElement);
        }
    ...
    function waitForGrid() {
      ...
      if (grid) {
        ...
        if ("IntersectionObserver" in window) {
          const visibilityObserver = new IntersectionObserver(function (
            entries
          ) {
