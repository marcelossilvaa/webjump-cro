(function () {
  ...
  const AC_SKUS = ["7069.80", "7083.80", "7990.90"]
  // ========================================

  // ========== SKUs DE ACESSÓRIOS GATILHO ==========
  const TRAVEL_MUG_SKUS = [
    "136877", // Travel Mug M Orange
    "137069", // TOUCH Travel Mug New SKU
    "142170", // Travel Mug S Sage Green
    "142171", // Travel Mug L Smokey Blue
    "142172", // Travel Mug M Vanilla Ice
    "150331", // Travel Mug M Black
    "158053", // Travel Mug M Lilac
  ];
  // ================================================
  ...
  async function detectCartTechnology() {
    ...
    // Travel Mug no carrinho tem prioridade → exibe AC_SKUS
    const hasTravelMug = userItems.some(function (item) {
      var sku = extractSkuFromProductId(item.productId);
      return TRAVEL_MUG_SKUS.includes(sku);
    });
    if (hasTravelMug) return "AC";

    // Verifica cápsulas e máquinas para determinar tecnologia
    var hasOL = false;
    var hasVL = false;
    ...
    for (var i = 0; i < userItems.length; i++) {
      try {
        ...
        if (!product || (product.type !== "capsule" && product.type !== "machine")) continue;
        hasProduct = true;
