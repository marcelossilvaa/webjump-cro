...
function createBenefitsSection() {
  // Seleciona o container do dropdown
  const dropdownFath = document.querySelector('.LoginDropdown__dropdown');
  const dropdownContainer = document.querySelector('.LoginDropdown__container');
  ...
  // Verifica se a seção já existe para evitar duplicação
  if (document.querySelector('.LoginDropdown__benefits')) {
    console.log('Seção de benefícios já existe');
    return;
  }
...
// Também executa quando o dropdown for aberto (caso seja carregado dinamicamente)
const benefitsObserver = new MutationObserver(function (mutations) {
  mutations.forEach(function (mutation) {
    if (mutation.type === 'childList') {
      const dropdown = document.querySelector('.LoginDropdown__dropdown');
      if (dropdown && !document.querySelector('.LoginDropdown__benefits')) {
        createBenefitsSection();
      }
    }
  });
...
// Observa mudanças no body para detectar quando o dropdown aparece
benefitsObserver.observe(document.body, {
  childList: true,
  subtree: true,
});
