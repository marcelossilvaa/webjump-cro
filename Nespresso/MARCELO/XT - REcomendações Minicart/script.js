(function() {
  ...
  // ========== SKUs RECOMENDADOS ==========
  // Gatilho do carrinho → lista de cafes recomendados:
  //   Acessorio          → AC_SKUS (prioridade)
  //   Maquina/cafe OL    → OL_SKUS
  //   Maquina/cafe VL    → VL_SKUS
  const OL_SKUS = [
      "7990.90", // Florian
      "7051.90", // Vienna Decaffeinato
      "7882.90", // Paris Espresso
  ];
  const VL_SKUS = [
      "7078.80", // Sweet Almond & Hibiscus
      "7043.80", // Costa Rica
      "7017.80", // Ethiopia
  ];
  const AC_SKUS = [
      "7069.80", // White Chocolate & Strawberry
      "7078.80", // Sweet Almond & Hibiscus
      "7990.90", // Florian
  ];
  // ========================================
...
`;
  ...
  async function detectCartTechnology() {
      ...
      // Gatilho amplo: cafes, maquinas e acessorios no carrinho
      let hasOL = false;
      let hasVL = false;
      let hasAccessory = false;
      let hasCapsuleOrMachine = false;

      for (let i = 0; i < userItems.length; i++) {
          try {
              const sku = extractSkuFromProductId(userItems[i].productId);
              if (!sku) continue;
              ...
              if (product.type !== "capsule" && product.type !== "machine") continue;

              hasCapsuleOrMachine = true;
              const tech = (product.technologies && product.technologies[0]) || "";
      ...
      // Acessorio tem prioridade → AC_SKUS
      if (hasAccessory) return "AC";
      if (!hasCapsuleOrMachine) return null;
      // Apenas VL no carrinho → VL; qualquer outro caso (OL ou misto) → OL
      if (hasVL && !hasOL) return "VL";
      return "OL";
  }

  function getRecommendedSkus(tech) {
      if (tech === "AC") return AC_SKUS;
  ...
  const buildPanel = async () => {
      ...
      // Sem cafe, maquina ou acessorio no carrinho → remove painel
