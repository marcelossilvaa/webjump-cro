(function () {
  ...
  const CARDS_CONFIG = [
    ...
    {
      id: "promocao-capsulas",
      enabled: true,
      position: 3,
      targetCollections: ["barista-creation"],
      filterCondition: "0",
      hasBackground: true,
      variants: {
        ol: {
          ...
          // Configuração para adicionar ao carrinho ao invés de link
          ctaAction: "addToCart",
          ctaSKU: "7894.90",
          ctaQuantity: 10,
        },
  ...
  function addToCart(sku, quantity, button, cardId) {
    ...
    try {
      ...
      if (
        ...
        // Chamar a função do CartManager
        window.CartManager.updateItem(sku, quantity, null, null, false);

        // Enviar evento GA4 para adicionar ao carrinho
  ...
  function createCardElement(cardConfig, templateArticle) {
    ...
    // NOVO: Verificar se é botão de adicionar ao carrinho ou link normal
    if (
      ...
      // Criar link normal (comportamento original)
      htmlParts.push(
        `
        <a href="` +
    ...
    // Event tracking para CTA link (comportamento original)
    ...
    // NOVO: Event handler para botão de adicionar ao carrinho
    const ctaButton = cardDetails.querySelector(".btnCardCrossSell");
    if (ctaButton) {
  ...
  function removeCard(cardId) {
    ...
    // Remover do tracking quando card é removido
  ...
  function updateCards() {
    ...
    if (filterChanged) {
      ...
      // Limpar eventos de carregamento enviados quando filtro muda
