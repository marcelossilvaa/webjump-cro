(function () {
  ...
  function getMiniCartCss() {
    return [
      ...
      ".custom-view-cart-btn { display: block; width: 100%; padding: 11px 20px; margin: 0px 0px 16px; background-color: #173C56; color: #ffffff; font-size: 18px; font-weight: 700; text-align: center; text-decoration: none; border-radius: 100px; border: none; cursor: pointer; transition: all 0.3s ease; box-sizing: border-box; }",
  ...
  function createViewCartButton($) {
    ...
    if (actionsContainer.length && !actionsContainer.find('.custom-view-cart-btn').length) {
      const viewCartBtn = '<button type="button" class="custom-view-cart-btn">Finalizar Compra</button>';
      actionsContainer.prepend(viewCartBtn);
    }
    $('.custom-view-cart-btn').each(function () {
  ...
  function createContinueShoppingButton($) {
    ...
    if (actionsContainer.length && !actionsContainer.find('.custom-continue-shopping-btn').length) {
      ...
      const continueBtn = '<a href="https://www.lojafamilynes.com.br/checkout/cart/" class="custom-continue-shopping-btn">Ir para o carrinho</a>';
