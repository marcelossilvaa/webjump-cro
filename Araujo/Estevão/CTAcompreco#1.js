(function () {
  ...
  const STYLE_ID = 'at-cta-preco-pdp-style';
  const DATA_ATTR = 'data-at-cta-preco-pdp-added';
  const ACTIVE_ATTR = 'data-at-cta-preco-pdp-active';
  const POST_ADD_ATTR = 'data-at-cta-preco-post-add';

  const INFO_CLASS = 'at-cta-preco-info';
  const ROW_CLASS = 'at-cta-preco-row';
  const OLD_PRICE_CLASS = 'at-cta-preco-old';
  const DISCOUNT_CLASS = 'at-cta-preco-discount';
  const PRICE_CLASS = 'at-cta-preco-current';
  const INSTALLMENTS_CLASS = 'at-cta-preco-installments';
  const INSTALLMENTS_NOWRAP_CLASS = 'at-cta-preco-installments-nowrap';
  ...
  function buildInfoBlock(priceContainer) {
    ...
    if (installmentsText) {
      const installmentsEl = document.createElement('span');
      installmentsEl.className = INSTALLMENTS_CLASS;
      if (oldPriceText || discountText) {
        installmentsEl.className += ' ' + INSTALLMENTS_NOWRAP_CLASS;
      }
      installmentsEl.textContent = installmentsText;
      info.appendChild(installmentsEl);
    }
  ...
  function setupObserver() {
    if (window._ctaPrecoPdpObserver) return;
    ...
    window._ctaPrecoPdpObserver = observer;
  }
